...
CREATE TABLE IF NOT EXISTS gl_code_line (
    id Int64 CODEC(T64, ZSTD(1)),
    traversal_path String CODEC(ZSTD(1)),
    project_id Int64 CODEC(T64, ZSTD(1)),
    branch String CODEC(ZSTD(1)),
    ...
    line_number Int64 DEFAULT 0 CODEC(Delta(8), ZSTD(1)),
    ...
    INDEX idx_content content TYPE text(tokenizer = splitByNonAlpha) GRANULARITY 1
) ENGINE = ReplacingMergeTree(_version, _deleted)
ORDER BY (traversal_path, project_id, branch, id)
SETTINGS index_granularity = 8192, allow_experimental_replacing_merge_with_cleanup = 1, auto_statistics_types = 'minmax, uniq, countmin';
