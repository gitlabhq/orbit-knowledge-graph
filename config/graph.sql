-- SCHEMA_VERSION=25 [AUTOGENERATED — DO NOT EDIT MANUALLY]
...
CREATE TABLE IF NOT EXISTS gl_definition (
    ...
    INDEX idx_fqn_ngram fqn TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
    INDEX idx_file_path file_path TYPE text(tokenizer = splitByString(['/'])) GRANULARITY 1,
    INDEX idx_file_path_ngram file_path TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
...
CREATE TABLE IF NOT EXISTS gl_deployment (
    ...
    INDEX idx_ref_ngram ref TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
    PROJECTION by_id (SELECT * ORDER BY id),
...
CREATE TABLE IF NOT EXISTS gl_directory (
    ...
    INDEX idx_path_ngram path TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
    PROJECTION by_id (SELECT * ORDER BY id),
    PROJECTION tp_count (
      SELECT traversal_path, uniq(id)
      GROUP BY traversal_path
...
CREATE TABLE IF NOT EXISTS gl_environment (
    ...
    INDEX idx_environment_type_ngram environment_type TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
    INDEX idx_id id TYPE bloom_filter(0.01) GRANULARITY 1,
    INDEX idx_name name TYPE text(tokenizer = splitByNonAlpha) GRANULARITY 1,
    INDEX idx_name_ngram name TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
    PROJECTION by_id (SELECT * ORDER BY id),
...
CREATE TABLE IF NOT EXISTS gl_finding (
    ...
    INDEX idx_description_ngram description TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
    PROJECTION by_id (SELECT * ORDER BY id),
    PROJECTION tp_count (
      SELECT traversal_path, uniq(id)
      GROUP BY traversal_path
...
CREATE TABLE IF NOT EXISTS gl_group (
    ...
    INDEX idx_full_path_ngram full_path TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
    INDEX idx_description description TYPE text(tokenizer = splitByNonAlpha) GRANULARITY 1,
    INDEX idx_description_ngram description TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
    PROJECTION by_id (SELECT * ORDER BY id),
    PROJECTION tp_count (
...
CREATE TABLE IF NOT EXISTS gl_imported_symbol (
    ...
    INDEX idx_identifier_name_ngram identifier_name TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
    INDEX idx_file_path file_path TYPE text(tokenizer = splitByString(['/'])) GRANULARITY 1,
    INDEX idx_file_path_ngram file_path TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
    INDEX idx_import_path import_path TYPE text(tokenizer = splitByNonAlpha) GRANULARITY 1,
    INDEX idx_import_path_ngram import_path TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
...
CREATE TABLE IF NOT EXISTS gl_label (
    ...
    INDEX idx_title_ngram title TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
    INDEX idx_description description TYPE text(tokenizer = splitByNonAlpha) GRANULARITY 1,
    INDEX idx_description_ngram description TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
    PROJECTION by_id (SELECT * ORDER BY id),
    PROJECTION tp_count (
...
CREATE TABLE IF NOT EXISTS gl_merge_request (
    ...
    INDEX idx_source_branch_ngram source_branch TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
    INDEX idx_target_branch target_branch TYPE text(tokenizer = splitByNonAlpha) GRANULARITY 1,
    INDEX idx_target_branch_ngram target_branch TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
...
CREATE TABLE IF NOT EXISTS gl_merge_request_diff_file (
    ...
    INDEX idx_new_path_ngram new_path TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
    INDEX idx_old_path old_path TYPE text(tokenizer = splitByString(['/'])) GRANULARITY 1,
    INDEX idx_old_path_ngram old_path TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
    PROJECTION by_id (SELECT * ORDER BY id),
    PROJECTION tp_count (
      SELECT traversal_path, uniq(id)
      GROUP BY traversal_path
...
CREATE TABLE IF NOT EXISTS gl_note (
    ...
    INDEX idx_note_ngram note TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
...
CREATE TABLE IF NOT EXISTS gl_user (
    ...
    INDEX idx_username_ngram username TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
    INDEX idx_name name TYPE text(tokenizer = splitByNonAlpha) GRANULARITY 1,
    INDEX idx_name_ngram name TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1
) ENGINE = ReplacingMergeTree(_version, _deleted)
ORDER BY (id) PRIMARY KEY (id)
...
CREATE TABLE IF NOT EXISTS gl_vulnerability_identifier (
    ...
    INDEX idx_external_type_ngram external_type TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
    INDEX idx_external_id external_id TYPE text(tokenizer = splitByNonAlpha) GRANULARITY 1,
    INDEX idx_external_id_ngram external_id TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
    INDEX idx_name name TYPE text(tokenizer = splitByNonAlpha) GRANULARITY 1,
    INDEX idx_name_ngram name TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
    PROJECTION by_id (SELECT * ORDER BY id),
    PROJECTION tp_count (
...
CREATE TABLE IF NOT EXISTS gl_vulnerability_occurrence (
    ...
    INDEX idx_cve_ngram cve TYPE ngrambf_v1(3, 256, 2, 0) GRANULARITY 1,
    PROJECTION by_id (SELECT * ORDER BY id),
    PROJECTION tp_count (
      SELECT traversal_path, uniq(id)
      GROUP BY traversal_path
