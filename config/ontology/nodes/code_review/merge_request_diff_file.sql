SELECT
  ...
  file.{{watermark_column}} AS _version,
  file.{{deleted_column}} AS _deleted
FROM siphon_merge_request_diff_files file
INNER JOIN (
  SELECT
    id,
    merge_request_id,
  ...
  WHERE startsWith(traversal_path, {traversal_path:String}) ) diff ON file.merge_request_diff_id = diff.id
WHERE startsWith(file.traversal_path, {traversal_path:String}) {{filters}}
