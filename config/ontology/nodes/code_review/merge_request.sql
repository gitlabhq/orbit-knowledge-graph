-- Each JOIN narrows to merge_request_ids in the current watermark window so
-- per-batch memory is bounded by batch size, not namespace size (~2 GB else).
SELECT
    src.*,
...
FROM (
    SELECT
        ...
        m.{{watermark_column}} AS {{watermark_column}},
        m.{{deleted_column}} AS {{deleted_column}}
    FROM merge_requests AS m
    LEFT JOIN (
        SELECT
    ...
    WHERE startsWith(m.traversal_path, {traversal_path:String})
) AS src
WHERE 1=1 {{filters}}
