SELECT
  ...
  project.{{watermark_column}} AS _version,
...
INNER JOIN (
  ...
  WHERE startsWith(traversal_path, {traversal_path:String}) ) traversal_paths ON project.id = traversal_paths.id
LEFT JOIN (
  SELECT
  ...
  GROUP BY source_id ) route ON project.id = route.source_id
