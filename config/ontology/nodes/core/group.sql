SELECT
  ...
  namespace.{{watermark_column}} AS _version,
...
INNER JOIN (
  ...
  WHERE startsWith(traversal_path, {traversal_path:String}) ) traversal_paths ON namespace.id = traversal_paths.id
LEFT JOIN (
  SELECT
    namespace_id,
    argMax(description, {{watermark_column}}) AS description
  FROM siphon_namespace_details
  WHERE startsWith(traversal_path, {traversal_path:String})
  GROUP BY namespace_id ) details ON namespace.id = details.namespace_id
LEFT JOIN (
  SELECT
    toNullable(source_id) AS source_id,
    argMax(path, {{watermark_column}}) AS path
  FROM siphon_routes
  WHERE startsWith(traversal_path, {traversal_path:String})
    AND source_type = 'Namespace'
    AND NOT {{deleted_column}}
  GROUP BY source_id ) routes ON namespace.id = routes.source_id
WHERE startsWith(traversal_paths.traversal_path, {traversal_path:String})
