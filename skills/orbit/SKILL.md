...
All query types use `nodes` (an array). `neighbors` and single-node
`traversal` take exactly one node; multi-node `traversal`, `aggregation`, and
`path_finding` take 2+ nodes plus `relationships`.
