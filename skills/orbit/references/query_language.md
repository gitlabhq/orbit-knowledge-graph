...
| `limit` | `integer` | Maximum rows to return when `cursor` is omitted. Default 30. Maximum 1000. With `cursor`, `page_size` controls returned rows and the server fetches through `offset + page_size + 1` to detect truncation. |
