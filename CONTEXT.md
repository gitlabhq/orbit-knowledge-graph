...
**Campaign ID**:
An optional UUID that groups all dispatch cycles triggered by a single re-indexing decision (schema migration). Generated by the migration orchestrator, stored in `gkg_schema_version`, and read by dispatchers via shared `CampaignState`. Null during steady-state incremental indexing.
