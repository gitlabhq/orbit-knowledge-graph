...
5. Collect findings. Create a draft note for anything worth flagging. Use code suggestions when you have a concrete fix
6. Create a draft summary note, then bulk publish all drafts as a single review
...
Check `/tmp/refs/siphon` for CDC payload shapes and `/tmp/refs/gitlab` for Rails auth when needed.
...
On the Rails side (in `/tmp/refs/gitlab`), look under `ee/lib/analytics/knowledge_graph/` for JWT signing, authorization context, and the gRPC client. Batch authorization logic is in `app/services/authz/`.
...
- Redaction is fail-closed: Rails errors → deny access, never skip
