...
Reference repos at `/tmp/refs/`:
- `/tmp/refs/gitlab` — GitLab Rails monolith (data model, Ability checks)
- `/tmp/refs/clickhouse-docs` — ClickHouse docs (query optimization, table engines)
- `/tmp/refs/siphon` — Siphon CDC pipeline (upstream data source)
...
When the MR changes SQL generation or schema, reconstruct the generated SQL, and cross-reference `/tmp/refs/clickhouse-docs` for how ClickHouse handles it. SQL generation starts in `crates/query-engine/src/` and indexer in `crates/indexer/src/`. Cite the ClickHouse docs in your analysis.
