...
enum DebugCommands {
    ...
    /// Generate the post-backfill projection DDL applied after a migrating
    /// version's backfill completes. Output is the contents of
    /// `config/graph_projections.sql`.
    DdlProjections {
        /// Path to ontology directory (default: embedded)
        #[arg(long, short)]
        ontology: Option<PathBuf>,

        /// Table prefix (e.g., "v1_" for schema version 1). Defaults to
        /// empty for the checked-in snapshot.
        #[arg(long, short, default_value = "")]
        prefix: String,
    },
}
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Debug { command } => match command {
            ...
            DebugCommands::DdlProjections { ontology, prefix } => {
                run_ddl_projections(ontology, prefix)
            }
        },
    }
}
...
fn run_ddl(ontology_path: Option<PathBuf>, prefix: String, diff: Option<PathBuf>) -> Result<()> {
    ...
    let mut tables = query_engine::compiler::generate_graph_tables(&ont);
    // Strip projections so this snapshot matches what the runtime migration
    // emits at create time. Projections are added by the post-backfill
    // phase — see `orbit debug ddl-projections` and
    // `config/graph_projections.sql`.
    query_engine::compiler::strip_projections_for_create(&mut tables);
...
fn run_ddl_projections(ontology_path: Option<PathBuf>, prefix: String) -> Result<()> {
    let ont = match ontology_path {
        Some(path) => Ontology::load_from_dir(&path).context("failed to load ontology")?,
        None => Ontology::load_embedded().context("failed to load embedded ontology")?,
    };
    ...
    let schema_version = include_str!("../../../config/SCHEMA_VERSION").trim();
    let statements = query_engine::compiler::generate_post_backfill_statements(&ont, &prefix);

    println!("-- SCHEMA_VERSION={schema_version} [AUTOGENERATED — DO NOT EDIT MANUALLY]");
    println!("-- Regenerate with: mise schema:generate:ddl");
    println!("-- Applied by the indexer after a migrating version's backfill completes; see");
    println!("-- crates/indexer/src/schema/completion.rs::apply_deferred_projections.");
    println!();

    if statements.is_empty() {
        println!("-- (no projections declared in ontology)");
        return Ok(());
    }

    let mut current_table: Option<&str> = None;
    for stmt in &statements {
        if Some(stmt.table.as_str()) != current_table {
            if current_table.is_some() {
                println!();
            }
            println!("-- {} ----------------------------", stmt.table);
            current_table = Some(stmt.table.as_str());
        }
        println!("{};", stmt.sql);
    }
    Ok(())
