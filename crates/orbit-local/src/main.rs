...
enum DebugCommands {
    ...
    /// Generate the baseline (first) migration from the ontology: the whole
    /// graph schema as `CREATE` statements, with a `down` that drops it.
    Migration {
        /// Path to ontology directory (default: embedded)
        #[arg(long, short)]
        ontology: Option<PathBuf>,
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Debug { command } => match command {
            ...
            DebugCommands::Migration { ontology } => run_migration(ontology),
        },
    }
}
...
/// Emits the baseline (first) migration: every graph table and dictionary as
/// `CREATE` statements (the `up`), plus a `down` that drops them. The first
/// migration diffs against an empty schema, so everything is new and no breaking
/// change is possible.
fn run_migration(ontology_path: Option<PathBuf>) -> Result<()> {
    let ont = match ontology_path {
        Some(path) => Ontology::load_from_dir(&path).context("failed to load ontology")?,
        None => Ontology::load_embedded().context("failed to load embedded ontology")?,
    };
    ...
    let diff =
        migration_framework::generation::generate_from_ontology(&[], &ont).map_err(|breaking| {
            anyhow::anyhow!("baseline migration cannot contain breaking changes: {breaking:?}")
        })?;

    let dictionaries = query_engine::compiler::generate_graph_dictionaries(&ont);

    let mut up = migration_framework::generation::render_up(&diff);
    up.extend(dictionaries.iter().map(|d| {
        format!(
            "{};",
            query_engine::compiler::emit_create_dictionary(d, &local_dictionary_source())
        )
    ...
    // Dictionaries read from tables, so drop them before the tables they depend on.
    let mut down: Vec<String> = dictionaries
        .iter()
        .map(|d| format!("DROP DICTIONARY IF EXISTS {};", d.name))
        .collect();
    down.extend(migration_framework::generation::render_down(&diff));

    let schema_version = include_str!("../../../config/SCHEMA_VERSION").trim();
    println!("-- Baseline migration [AUTOGENERATED — DO NOT EDIT MANUALLY]");
    println!("-- Full graph schema as of SCHEMA_VERSION={schema_version}.");
    println!("-- Regenerate with: mise schema:generate:migration");
    println!();
    println!("-- +up");
    for statement in &up {
        println!("{statement}");
