...
fn install_for(assistant: &AssistantSpec, target: &Target, report: &mut Report) -> Result<()> {
...
fn remove_for(assistant: &AssistantSpec, target: &Target, report: &mut Report) -> Result<()> {
    for merge in &assistant.json_merges {
        let (path, label) = target.resolve(&merge.file)?;
        if !path.exists() {
            continue;
