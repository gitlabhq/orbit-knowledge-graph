...
//!
//! Regression coverage:
//! - `ensure_kv_bucket_exists` migrates an existing KV bucket in place rather
//!   than erroring with `STREAM_NAME_IN_USE` when configs differ.
//! - `create_or_update_stream` honors the per-call `max_age_override`,
//!   letting the dead-letter stream pin `max_age=0` while work-queue streams
...
/// Confirms `create_or_update_stream` respects the per-call max_age override.
/// A work-queue caller passes `None` (use config default), the dead-letter
/// caller passes `Some(Duration::ZERO)` (never expire). Both must round-trip
/// to the right server-side `max_age` even when both share the same client.
#[tokio::test]
