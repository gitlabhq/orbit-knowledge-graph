...
pub struct QuotaConfig {
    /// Master switch for quota checks. The quota gate fires only when both
    /// the parent `billing.enabled` and this flag are true — the parent flag
    /// is the SOX-audited surface, this one is the operational kill-switch.
    #[serde(default)]
    pub enabled: bool,
    #[serde(default)]
