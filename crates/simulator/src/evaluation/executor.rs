...
impl QueryExecutor {
    ...
    pub async fn execute_query(&mut self, name: &str, query: &QueryDefinition) -> ExecutionResult {
        ...
        let final_sql =
            substitute_params_in_sql(&compiled.structural.sql, &compiled.structural.params);
        ...
        match self.execute_sql_with_sample(&final_sql).await {
            Ok((row_count, sample_rows, column_names)) => ExecutionResult::success(
                ...
                column_names,
                start.elapsed(),
                compiled.structural.sql,
                serde_json::to_value(&compiled.structural.params).unwrap_or_default(),
                Some(sampling_info),
            ),
    ...
    pub async fn execute_query_with_metadata(
        ...
        builder = builder
            .sql(compiled.structural.sql.clone())
