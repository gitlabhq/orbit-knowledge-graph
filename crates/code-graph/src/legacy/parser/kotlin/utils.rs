...
pub(in crate::legacy::parser::kotlin) fn get_child_by_any_kind<'a>(
    node: &AstNode<'a>,
    node_kinds: &[&str],
) -> Option<AstNode<'a>> {
    node.children()
...
pub(in crate::legacy::parser::kotlin) fn get_children_by_kind<'a>(
    node: &AstNode<'a>,
    node_kind: &str,
) -> Vec<AstNode<'a>> {
    node.children()
