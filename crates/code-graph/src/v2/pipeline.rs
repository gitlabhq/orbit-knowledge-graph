...
/// - **Phase 1** (parallel): `analyze()` → `FileAnalysis` per file, add defs/imports to graph
/// - **Merge**: write inferred return types to graph, then `finalize()`
/// - **Phase 2** (parallel): resolve pre-computed refs against the finalized graph
    ...
    fn process_files(
        ...
        let parse_results: Vec<(Option<FileInfo>, Option<StashedRefs>)> = files
            ...
            .map(|path| {
                ...
                let analysis = match spec.analyze(&source, path, language) {
                    Ok(a) => a,
                    Err(e) => {
                        errors.lock().unwrap().push(PipelineError {
                            file_path: path.clone(),
                ...
                total_defs.fetch_add(analysis.defs.len(), std::sync::atomic::Ordering::Relaxed);
                total_imports
                    .fetch_add(analysis.imports.len(), std::sync::atomic::Ordering::Relaxed);
        ...
        if !errors.is_empty() && parse_results.iter().all(|(info, _)| info.is_none()) {
            return Err(errors);
        }
        ...
        // ── Phase 2: resolve pre-computed refs (parallel) ──
