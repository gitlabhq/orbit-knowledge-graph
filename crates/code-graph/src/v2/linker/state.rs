...
impl<const N: usize> VerifiedMap<N> {
    ...
    /// Returns all entries matching the hash of `key`. The verifier is only
    /// called when the caller needs to filter beyond hash equality (e.g. by
    /// DefKind). Hash collisions (~10⁻⁹) are accepted as benign.
    pub fn lookup(
        &self,
        key: &str,
    ...
    /// Returns all entries matching the hash of `key` without verification.
    /// Use when the hash match is sufficient (e.g. FQN lookups where the
    /// caller already knows the exact key).
    pub fn lookup_unverified(&self, key: &str) -> &[NodeIndex] {
        match self.inner.get(&hash_name(key)) {
