...
fn def_owner_map(defs: &[JsDef]) -> HashMap<(usize, usize), (String, crate::utils::Range)> {
    defs.iter()
        .map(|def| {
            (
...
fn def_owner_fqn_map(defs: &[JsDef]) -> HashMap<String, crate::utils::Range> {
    defs.iter()
        .map(|def| (def.fqn.clone(), def.range))
        .collect()
}

fn structural_call_owner(
    ctx: &Ctx,
    def_owners: &HashMap<(usize, usize), (String, crate::utils::Range)>,
    def_owner_fqns: &HashMap<String, crate::utils::Range>,
    ...
    for ancestor_id in ctx.nodes.ancestor_ids(node_id).skip(1) {
        match ctx.nodes.kind(ancestor_id) {
            AstKind::MethodDefinition(method) => {
                ...
                let class_name = ctx.nodes.ancestor_ids(ancestor_id).skip(1).find_map(
                    |candidate_id| match ctx.nodes.kind(candidate_id) {
                ...
                if let Some(owner_range) = def_owner_fqns.get(&fqn) {
                    return Some((fqn, *owner_range));
                }
            }
...
fn resolve_caller_site(
    ...
    structural_call_owner(ctx, def_owners, def_owner_fqns, node_id)
...
pub(super) fn extract_call_edges(
    ...
    let def_owner_fqns = def_owner_fqn_map(defs);
