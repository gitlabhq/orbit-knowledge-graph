...
use super::parser::{RefKind, Reference};
...
impl ResolutionPlan {
        ...
        I: IntoIterator<Item = (&'a str, &'a Reference)>,
    {
        ...
        for (default_project, r) in refs {
            ...
            match r.kind {
                RefKind::Issue => {
                    plan.paths.insert(project.clone());
                    if let Some(iid) = r.iid {
                        plan.issue_pairs.insert((project, iid));
                    }
                }
                RefKind::MergeRequest => {
                    plan.paths.insert(project.clone());
                    if let Some(iid) = r.iid {
                        plan.mr_pairs.insert((project, iid));
                    }
                }
                RefKind::Commit => {
                    plan.commit_ref_count += 1;
                    if !r.project_path.as_deref().unwrap_or("").is_empty() {
                        plan.paths.insert(project);
                    }
                }
            }
        }
...
mod tests {
    use super::*;
    use crate::system_notes_bench::parser::{Action, extract};
