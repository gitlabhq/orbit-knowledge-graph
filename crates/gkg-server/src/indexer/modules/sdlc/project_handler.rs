...
WHERE project.id IN (SELECT id FROM project_namespace_traversal_paths WHERE traversal_path LIKE {traversal_path:String})
...
pub struct ProjectChildHandler {
    datalake: Arc<dyn DatalakeQuery>,
}

impl ProjectChildHandler {
    ...
    async fn transform_and_write_batch(
        ...
        // Transform and write creator edges
        ...
        // Transform and write contains edges
...
#[async_trait]
impl NamespaceChildHandler for ProjectChildHandler {
    fn name(&self) -> &str {
        "project-child-handler"
    }
    ...
    async fn handle(&self, context: &ChildHandlerContext) -> Result<(), HandlerError> {
        ...
        let mut stream = self
            .datalake
            .query_arrow(PROJECT_QUERY, Some(build_query_params(context)))
            .await
