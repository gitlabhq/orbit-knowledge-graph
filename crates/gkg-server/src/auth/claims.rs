...
pub struct Claims {
    ...
    /// Developer). The compiler security pass consumes it to filter paths
    /// per-entity: for example, a user with `[("1/2/", Reporter),
    /// ("1/3/", Developer)]` sees Project rows from both paths but only
    /// Vulnerability rows from `1/3/`.
    #[serde(default)]
