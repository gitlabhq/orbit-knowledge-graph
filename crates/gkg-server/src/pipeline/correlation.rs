...
/// The forwarded `x-gitlab-correlation-id` is untrusted upstream input, taken
/// verbatim, so it can hold URL-unsafe bytes that would corrupt the `query_id`
/// URL param; those fall back to a ULID and are preserved intact in `log_comment`.
