...
impl PipelineStage for CachedExecutor {
    ...
    async fn execute(
        ...
        let cache_enabled = config.app_cache_enabled.unwrap_or(false);
        if !cache_enabled {
            return ClickHouseExecutor.execute(ctx, obs).await;
        }

        let ttl_secs = config.app_cache_ttl.unwrap_or(60);
        ...
        // Cache miss -- execute against ClickHouse
        ...
        if let Ok(data) = serialize_batches(&output.batches) {
            if data.len() <= MAX_CACHE_VALUE_BYTES {
                ...
                tokio::spawn(async move {
                    let options = KvPutOptions::with_ttl(Duration::from_secs(ttl_secs as u64));
                    if let Err(e) = broker
