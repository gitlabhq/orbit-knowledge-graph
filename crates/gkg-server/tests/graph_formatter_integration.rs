...
fn find_edge<'a>(edges: &'a [Value], from: &str, to: &str, edge_type: &str) -> Option<&'a Value> {
    edges
        .iter()
        .find(|e| e["from"] == from && e["to"] == to && e["type"] == edge_type)
}
...
async fn search_exact_properties(ctx: &TestContext) {
    ...
    let value = run_pipeline(
        ...
        r#"{
            "query_type": "search",
            "node": {"id": "u", "entity": "User", "columns": ["username", "state", "name"]},
            "limit": 10
        }"#,
        &allow_all(),
    )
...
async fn traversal_variable_length_reaches_depth_2(ctx: &TestContext) {
    ...
    // Single-hop baseline: User→Group via MEMBER_OF only reaches groups 100,101,102
    let single_hop = run_pipeline(
        ctx,
        r#"{
            "query_type": "traversal",
            "nodes": [
    ...
    let single_hop_groups = node_ids(single_hop["nodes"].as_array().unwrap(), "Group");
    assert!(
        !single_hop_groups.contains(&200),
        "depth-2 group should NOT appear at 1 hop"
    );
    assert!(
        !single_hop_groups.contains(&300),
        "depth-3 group should NOT appear at 1 hop"
    );

    // Multi-hop: max_hops=3 follows User→Group→Group→Group chain
    // User 1 →MEMBER_OF→ Group 100 →MEMBER_OF→ Group 200 →MEMBER_OF→ Group 300
    let multi_hop = run_pipeline(
        ctx,
        r#"{
            "query_type": "traversal",
            "nodes": [
            ...
            "relationships": [{"type": "MEMBER_OF", "from": "u", "to": "g", "min_hops": 1, "max_hops": 3}],
            "limit": 50
        }"#,
        &allow_all(),
    )
    ...
    let multi_nodes = multi_hop["nodes"].as_array().unwrap();
    let multi_hop_groups = node_ids(multi_nodes, "Group");
    assert!(
        multi_hop_groups.contains(&100),
        "direct group must be present"
    );
    assert!(
        multi_hop_groups.contains(&200),
        "depth-2 group must be reachable via multi-hop"
    );
    assert!(
        multi_hop_groups.contains(&300),
        "depth-3 group must be reachable via multi-hop"
    );

    let depth2 = find_node(multi_nodes, "Group", 200);
    assert_eq!(depth2["name"].as_str().unwrap(), "Depth2 Group");
    assert!(depth2["id"].is_i64());
    let depth3 = find_node(multi_nodes, "Group", 300);
    assert_eq!(depth3["name"].as_str().unwrap(), "Depth3 Group");

    let edges = multi_hop["edges"].as_array().unwrap();
    assert!(edges.iter().all(|e| e["type"] == "MEMBER_OF"));
    assert!(edges.iter().all(|e| e["depth"].is_i64()));
...
async fn traversal_variable_length_min_hops_skips_shallow(ctx: &TestContext) {
    ...
    // min_hops=2: should only return groups reachable at depth ≥ 2
    // User 1 → Group 100 is depth 1 (skipped)
    // User 1 → Group 100 → Group 200 is depth 2 (included)
    // User 1 → Group 100 → Group 200 → Group 300 is depth 3 (included)
    let value = run_pipeline(
        ctx,
        r#"{
            "query_type": "traversal",
            ...
            "relationships": [{"type": "MEMBER_OF", "from": "u", "to": "g", "min_hops": 2, "max_hops": 3}],
            "limit": 50
        }"#,
        &allow_all(),
    )
    ...
    let nodes = value["nodes"].as_array().unwrap();
    let groups = node_ids(nodes, "Group");
    assert!(groups.contains(&200), "depth-2 group must be reachable");
    assert!(groups.contains(&300), "depth-3 group must be reachable");
    assert!(
        !groups.contains(&100),
        "depth-1 group must be excluded by min_hops"
    );

    let depth2 = find_node(nodes, "Group", 200);
    assert_eq!(depth2["name"].as_str().unwrap(), "Depth2 Group");
    let depth3 = find_node(nodes, "Group", 300);
    assert_eq!(depth3["name"].as_str().unwrap(), "Depth3 Group");

    let edges = value["edges"].as_array().unwrap();
    assert!(edges.iter().all(|e| e["depth"].is_i64()));
    assert!(!edges.iter().any(|e| e["depth"] == 1));
    assert!(
        edges
            .iter()
            .any(|e| e["from_id"] == 1 && e["to_id"] == 200 && e["depth"] == 2)
    );
    assert!(
        edges
            .iter()
            .any(|e| e["from_id"] == 1 && e["to_id"] == 300 && e["depth"] == 3)
    );
}

async fn traversal_multi_relationship_chain(ctx: &TestContext) {
    seed(ctx).await;
    ...
    assert!(users.contains(&1), "alice");
    assert!(groups.contains(&100), "Public Group");
    assert!(projects.contains(&1000), "Public Project");

    let alice = find_node(nodes, "User", 1);
    assert_eq!(alice["username"].as_str().unwrap(), "alice");
    let pub_group = find_node(nodes, "Group", 100);
    assert_eq!(pub_group["name"].as_str().unwrap(), "Public Group");
    let pub_project = find_node(nodes, "Project", 1000);
    assert_eq!(pub_project["name"].as_str().unwrap(), "Public Project");
    ...
    let member_of = find_edge(edges, "User", "Group", "MEMBER_OF").unwrap();
    assert!(member_of["from_id"].is_i64());
    assert!(member_of["to_id"].is_i64());
    let contains = find_edge(edges, "Group", "Project", "CONTAINS").unwrap();
    assert!(contains["from_id"].is_i64());
    assert!(contains["to_id"].is_i64());
...
async fn traversal_three_entity_chain(ctx: &TestContext) {
    seed(ctx).await;

    // User →MEMBER_OF→ Group →CONTAINS→ Project (3 entities, 2 relationships)
    let value = run_pipeline(
        ctx,
        r#"{
            "query_type": "traversal",
    ...
    assert!(groups.contains(&100));
    assert!(groups.contains(&101));
    assert!(projects.contains(&1000));
    assert!(projects.contains(&1001));
    ...
    assert_eq!(priv_group["name"].as_str().unwrap(), "Private Group");
    let pub_project = find_node(nodes, "Project", 1000);
    assert_eq!(pub_project["name"].as_str().unwrap(), "Public Project");
    let priv_project = find_node(nodes, "Project", 1001);
    assert_eq!(priv_project["name"].as_str().unwrap(), "Private Project");

    let edges = value["edges"].as_array().unwrap();
    let member_of_edges: Vec<_> = edges.iter().filter(|e| e["type"] == "MEMBER_OF").collect();
    let contains_edges: Vec<_> = edges.iter().filter(|e| e["type"] == "CONTAINS").collect();
    ...
    assert!(
        edges
            .iter()
            .any(|e| e["from_id"] == 1 && e["to_id"] == 100 && e["type"] == "MEMBER_OF"),
        "alice->Public Group MEMBER_OF edge"
    );
    assert!(
        edges
            .iter()
            .any(|e| e["from_id"] == 100 && e["to_id"] == 1000 && e["type"] == "CONTAINS"),
        "Public Group->Public Project CONTAINS edge"
    );
...
async fn traversal_variable_length_with_redaction_at_depth(ctx: &TestContext) {
    ...
    // Redact group 200 → depth-3 group 300 should also disappear since
    // the path goes through 200
    let mut svc = MockRedactionService::new();
    svc.allow("user", &[1, 2, 3, 4, 5]);
    svc.allow("group", &[100, 101, 102, 300]); // 200 NOT allowed
    ...
    let nodes = value["nodes"].as_array().unwrap();
    let groups = node_ids(nodes, "Group");
    assert!(groups.contains(&100), "direct group should survive");
    assert!(!groups.contains(&200), "redacted group 200 must not appear");
    assert!(
        !groups.contains(&300),
        "group 300 unreachable through redacted 200"
    );
...
// Two relationships targeting the same node (fan-in / diamond pattern).
// Before the fix, this generated SQL with duplicate table aliases:
//   INNER JOIN gl_note AS n ... INNER JOIN gl_note AS n ...
// causing ClickHouse error: "Multiple table expressions with same alias".
async fn traversal_shared_target_node(ctx: &TestContext) {
    seed(ctx).await;

    // Add edges so both User and Project connect to Note via different relationships
    ctx.execute(
        "INSERT INTO gl_edge (traversal_path, source_id, source_kind, relationship_kind, target_id, target_kind) VALUES
    ...
    // User →AUTHORED→ Note, Project →CONTAINS→ Note (both target "n")
    let value = run_pipeline(
        ctx,
        r#"{
            "query_type": "traversal",
...
async fn aggregation_count_exact(ctx: &TestContext) {
    ...
    assert_eq!(cols[0]["name"], "group_count");
...
async fn aggregation_column_metadata(ctx: &TestContext) {
    seed(ctx).await;
    ...
    let value = run_pipeline(
        ...
        r#"{
            ...
            "aggregations": [
                {"function": "count", "target": "p", "group_by": "g", "alias": "project_count"}
            ],
            "limit": 10
        }"#,
        &allow_all(),
    ...
    let cols = value["columns"].as_array().unwrap();
    assert_eq!(cols.len(), 1);
    assert_eq!(cols[0]["name"], "project_count");
    assert_eq!(cols[0]["type"], "Int64");
    assert_eq!(cols[0]["aggregation"], "count");
    ...
    for node in nodes {
        assert!(
            node["project_count"].is_i64(),
            "aggregation value must be integer"
        );
        assert!(node["name"].is_string(), "name property must be string");
        assert!(node["id"].is_i64());
    }

    let g100 = find_node(nodes, "Group", 100);
    assert_eq!(g100["name"].as_str().unwrap(), "Public Group");
    assert_eq!(g100["project_count"].as_i64().unwrap(), 1);
    let g101 = find_node(nodes, "Group", 101);
    assert_eq!(g101["name"].as_str().unwrap(), "Private Group");
    assert_eq!(g101["project_count"].as_i64().unwrap(), 1);
    let g102 = find_node(nodes, "Group", 102);
    assert_eq!(g102["name"].as_str().unwrap(), "Internal Group");
    assert_eq!(g102["project_count"].as_i64().unwrap(), 1);
...
async fn path_finding_exact_path(ctx: &TestContext) {
    ...
    assert!(value["row_count"].as_i64().unwrap() >= 1);
    assert!(value["columns"].as_array().unwrap().is_empty());
...
async fn path_finding_max_depth(ctx: &TestContext) {
    ...
    assert_eq!(value["query_type"], "path_finding");
    if value["row_count"].as_i64().unwrap() > 0 {
        let edges = value["edges"].as_array().unwrap();
        let max_step = edges
            .iter()
            .map(|e| e["step"].as_i64().unwrap())
...
async fn neighbors_both_exact(ctx: &TestContext) {
    ...
    assert!(!edges.iter().any(|e| {
        e["from"] == "Group" && e["from_id"] == 100 && e["to"] == "User" && e["type"] == "MEMBER_OF"
    }));
...
async fn giant_string_survives_pipeline(ctx: &TestContext) {
    ...
    if value["row_count"].as_i64().unwrap() > 0 {
        let nodes = value["nodes"].as_array().unwrap();
        if let Some(note) = nodes
            .iter()
            .find(|n| n["type"] == "Note" && n["id"] == 3001)
        {
            if let Some(text) = note.get("note") {
...
async fn aggregation_sum(ctx: &TestContext) {
    ...
    let cols = value["columns"].as_array().unwrap();
    assert_eq!(cols.len(), 1);
    assert_eq!(cols[0]["name"], "id_sum");
    assert_eq!(cols[0]["type"], "Int64");
    assert_eq!(cols[0]["aggregation"], "sum");
...
async fn aggregation_avg(ctx: &TestContext) {
    ...
    let cols = value["columns"].as_array().unwrap();
    assert_eq!(cols.len(), 1);
    assert_eq!(cols[0]["name"], "avg_id");
    assert_eq!(cols[0]["type"], "Float64");
    assert_eq!(cols[0]["aggregation"], "avg");
...
async fn aggregation_min_max(ctx: &TestContext) {
    ...
    assert!(col_names.contains(&"min_id"));
    assert!(col_names.contains(&"max_id"));
...
async fn aggregation_min_string(ctx: &TestContext) {
    ...
    assert_eq!(cols[0]["aggregation"], "min");
...
async fn aggregation_multiple_functions(ctx: &TestContext) {
    ...
    let count_col = cols.iter().find(|c| c["name"] == "member_count").unwrap();
    assert_eq!(count_col["type"], "Int64");
    assert_eq!(count_col["aggregation"], "count");

    let avg_col = cols.iter().find(|c| c["name"] == "avg_id").unwrap();
    assert_eq!(avg_col["type"], "Float64");
    assert_eq!(avg_col["aggregation"], "avg");

    let min_col = cols.iter().find(|c| c["name"] == "min_id").unwrap();
    assert_eq!(min_col["type"], "Int64");
    assert_eq!(min_col["aggregation"], "min");
...
async fn path_finding_with_rel_types(ctx: &TestContext) {
    ...
    // Path User→Project needs MEMBER_OF + CONTAINS — restricting to MEMBER_OF only should block it
    assert_eq!(
        value["row_count"], 0,
...
async fn graph_formatter_e2e() {
    ...
    run_subtests!(
        ...
        traversal_multi_relationship_chain,
        traversal_three_entity_chain,
        ...
        // Traversal — shared target (fan-in)
