//! Substrait encode: `v2::Plan` → `substrait::proto::Plan`.
//!
//! This is the only file in v2 that imports the `substrait` crate.
//! The encoder walks the `Rel`/`Expr` tree, tracks schemas for positional
//! column resolution, and produces the verbose Substrait protobuf structures.
//!
//! Used by the DataFusion backend which natively consumes Substrait plans.

use std::collections::HashMap;

use substrait::proto::{
    self, aggregate_rel, expression, fetch_rel, plan_rel, read_rel, rel, rel_common, set_rel,
...
use crate::ir::expr::{DataType, Expr, LiteralValue};
use crate::ir::substrait::{
    binary_op_substrait_name, bool_type, build_named_struct, make_any, make_literal_arg,
    make_metadata, make_scalar_fn, make_value_arg, string_type, to_substrait_join_type,
    to_substrait_literal, to_substrait_type, unary_op_substrait_name,
};
use crate::v2::plan::{self as v2, Measure, Plan, RawFrom};
...
// Reuse from substrait.rs
fn make_field_ref(index: usize) -> Expression {
    crate::ir::substrait::make_field_ref(index)
}
...
mod tests {
    use super::*;
    use crate::ir::expr::*;
    use crate::v2::plan::{CteDef, Measure, Rel as V2Rel};
