...
use arrow::array::{
    Array, ArrayBuilder, ArrayRef, BooleanArray, BooleanBuilder, ByteView, Date32Array,
    Date64Array, Float64Array, GenericListArray, GenericStringArray, Int8Array, Int16Array,
    Int32Array, Int64Array, Int64Builder, LargeStringArray, ListArray, OffsetSizeTrait,
    PrimitiveArray, StringArray, StringBuilder, StringViewArray, StructArray,
...
/// Version of the [`logical_byte_size`] counting rules below. Bump whenever a rule
/// changes, so historical data can be attributed to the rules that produced it.
pub const LOGICAL_SIZE_FORMULA_VERSION: u32 = 1;

/// A column's Arrow type has no counting rule defined in [`logical_byte_size`]. Returned
/// instead of silently counting the column as 0, so a new ontology column type can't ship
/// unmetered.
...
/// Deterministic count of the logical bytes in a [`RecordBatch`]: a pure function of the
/// logical values it carries, invariant under batch splitting, buffer capacity, dictionary
/// vs. plain string encoding, and Arrow library version. Feeds a downstream billing meter —
/// a customer can recompute the same count from the row values alone.
///
/// This counts only bytes of customer data. Serialization overhead — offset arrays, null
/// maps, size streams — is a storage-engine constraint, not something the customer put in,
/// so it is deliberately excluded even though ClickHouse's own `byteSize()` includes it.
///
/// Counting rules (bump [`LOGICAL_SIZE_FORMULA_VERSION`] to change any row):
///
...
/// | NULL (any type, incl. an all-null `Null`-typed column)| 0                            |
...
/// True if [`logical_byte_size`] can count this Arrow type. Used to check ontology-derived
/// column types have counting-rule coverage before any batch built from them reaches a writer.
...
fn value_byte_count(
    ...
    match data_type {
        // Every value in a `Null`-typed array is null by construction (e.g. DataFusion's
        // make_array() with no arguments), so this is the NULL-charges-0 rule degenerating
        // to a whole column rather than a new type to price.
        DataType::Null => Ok(0),
...
/// `width * (len - null_count)`: fixed-width null slots carry no customer data, so they
/// count for 0 rather than their allocated width. `null_count()` is a cached property of
/// the array, so this stays O(1) regardless of row count.
fn non_null_count(array: &dyn Array) -> u64 {
    (array.len() - array.null_count()) as u64
}

/// UTF-8 payload length of a sliced `Utf8`/`LargeUtf8` array, computed from its offset
/// window rather than `values().len()` so a slice counts only its own rows. Null slots
/// contribute 0 automatically: Arrow string builders always emit a zero-length value for
/// a null, so the offset window spans no bytes there.
...
/// `Utf8View` has no contiguous offset buffer to diff, so this sums per-row lengths of
/// non-null rows instead. Reads the length out of each inline 16-byte view header rather
/// than calling `.value(i)`, which would dereference the out-of-line buffer for every value
/// over 12 bytes — the difference between a cache-friendly scan and paging in the full
/// string heap per row.
...
/// Counts a `Dictionary(<int key>, Utf8)` column as its decoded string values would count,
/// without materializing them: looks up each occurring row's length directly in the
/// (already deduplicated) dictionary values array instead of casting to a fully expanded
/// `Utf8` array, which would reallocate and copy every repeated string. `keys_iter` yields
/// `None` for a null row, and `.flatten()` drops it, so nulls contribute 0.
...
/// Sums element counts over the (sliced) values array; an empty or null list spans a
/// zero-length offset window, so it recurses to 0 with no special case.
...
#[cfg(test)]
mod logical_byte_size_tests {
    use super::*;
    use arrow::array::{ListBuilder, StringDictionaryBuilder};
    ...
    fn all_counted_types_batch() -> RecordBatch {
        let int64 = Int64Array::from(vec![1, 2, 3, 4, 5]);
        let uint64 = UInt64Array::from(vec![10, 20, 30, 40, 50]);
        let ts = TimestampMicrosecondArray::from(vec![100, 200, 300, 400, 500]);
        let int32 = Int32Array::from(vec![1, 2, 3, 4, 5]);
        let uint32 = UInt32Array::from(vec![1, 2, 3, 4, 5]);
        let date32 = Date32Array::from(vec![1, 2, 3, 4, 5]);
        let boolean = BooleanArray::from(vec![true, false, true, false, true]);
        let str_col = StringArray::from(vec![Some("a"), None, Some(""), Some("bcd"), None]);
        let large_str = LargeStringArray::from(vec!["e", "ff", "", "hhhh", "i"]);
        let str_view = StringViewArray::from(vec!["j", "kk", "", "mmmm", "n"]);
    ...
    #[test]
    fn split_invariant_across_every_counted_type() {
        let batch = all_counted_types_batch();
        let total = logical_byte_size(&batch).unwrap();
        for split in 1..batch.num_rows() {
            let left = batch.slice(0, split);
            let right = batch.slice(split, batch.num_rows() - split);
            let sum = logical_byte_size(&left).unwrap() + logical_byte_size(&right).unwrap();
            assert_eq!(
                sum, total,
                "split at row {split} must not change the total byte count"
            );
        }
    }
    ...
    fn dictionary_counts_same_as_plain_utf8() {
        ...
        assert_eq!(
            logical_byte_size(&plain).unwrap(),
            logical_byte_size(&dict).unwrap()
        );
    }

    #[test]
    fn sliced_string_array_counts_only_its_rows() {
        let arr = StringArray::from(vec!["a", "bb", "ccc", "dddd"]);
        let batch = single_col_batch("s", DataType::Utf8, false, Arc::new(arr));

        let sliced = batch.slice(1, 2);
        assert_eq!(
            logical_byte_size(&sliced).unwrap(),
            "bb".len() as u64 + "ccc".len() as u64
        );
    }

    #[test]
    fn null_charges_zero_for_every_type() {
        let strings = single_col_batch(
            "s",
            DataType::Utf8,
            true,
            Arc::new(StringArray::from(vec![Some("hi"), None])),
        );
        assert_eq!(logical_byte_size(&strings.slice(1, 1)).unwrap(), 0);

        let ints = single_col_batch(
            "n",
            DataType::Int64,
            true,
            Arc::new(Int64Array::from(vec![Some(1i64), None])),
        );
        assert_eq!(logical_byte_size(&ints.slice(1, 1)).unwrap(), 0);

        let mut list_builder = ListBuilder::new(StringBuilder::new());
        list_builder.values().append_value("kept");
        list_builder.append(true);
        ...
        assert_eq!(logical_byte_size(&lists.slice(1, 1)).unwrap(), 0);

        let nulls = single_col_batch(
            "u",
            DataType::Null,
            true,
            Arc::new(arrow::array::NullArray::new(3)),
        );
        assert_eq!(logical_byte_size(&nulls).unwrap(), 0);
        assert!(is_counted(&DataType::Null));
    }

    #[test]
    fn interleaved_nulls_do_not_perturb_string_offset_payload() {
        let arr = StringArray::from(vec![Some("a"), None, Some("bb"), None, Some("ccc")]);
        let batch = single_col_batch("s", DataType::Utf8, true, Arc::new(arr));
        assert_eq!(logical_byte_size(&batch).unwrap(), 1 + 2 + 3);
    }

    #[test]
    fn uncounted_type_names_column_and_type() {
        let arr = Float64Array::from(vec![1.0, 2.0]);
        let batch = single_col_batch("score", DataType::Float64, false, Arc::new(arr));
