...
pub async fn uses_watermark_for_incremental_processing(context: &TestContext) {
    // Seed per-entity position with a completed watermark at 2024-01-19.
    // In v2, position is stored per entity: key = "ns.{namespace_id}.{entity_name}".
    context
        .execute(
    ...
    let result = context.query("SELECT count() as cnt FROM gl_group FINAL").await;
