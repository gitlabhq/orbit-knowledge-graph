...
- **Siphon columns:** hand-written datalake SQL must splice
  `ontology::constants::SIPHON_WATERMARK_COLUMN` (`_siphon_watermark`) and
  `ontology::constants::SIPHON_DELETED_COLUMN` (`_siphon_deleted`) via `const_format::concatcp!`,
  never the literal column names. Both consts are generated by `crates/ontology/build.rs` from
  `schema.yaml`'s `settings.etl.default_watermark` / `default_deleted`, so the ontology YAML is the
  single source of truth. Ontology YAML uses the `{{watermark_column}}` and `{{deleted_column}}`
  placeholders.
