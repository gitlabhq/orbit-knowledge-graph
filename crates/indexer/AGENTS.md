...
| Retry harness | `engine/retry.rs` | Shared retry vocabulary: `RetryPolicy` (mode + `Backoff` + cap), `Step`, and the `drive`/`drive_with`/`drive_until` in-situ executors. Global (NATS-redelivery) mode is executed by `run_handlers`, which reads the same policy |
