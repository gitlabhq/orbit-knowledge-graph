...
| ClickHouse writer | `clickhouse/` | `ClickHouseWriter` (one Arrow-IPC `write`) + `BufferedWriter` (per-table coalescing across all producers + local write retry). A table flushes at `write_slice_rows`, on the soft `write_buffer_age_secs` tick once it holds `write_min_flush_rows`, or when its oldest row exceeds the hard `write_max_flush_age_secs` cap — so a trickle of small repos pools into one part instead of one tiny part per tick |
...
The **dispatcher** publishes the current ontology archive to a durable, graph-database-scoped NATS KV catalog before migration. Identical publication retries succeed; conflicting bytes for a published version fail. The webserver serves the database’s active archive during backfill and switches snapshots in-process after promotion.
...
pipelines must also be complete. The checker verifies the incoming archive, promotes the `migrating` version to `active` and retires the outgoing version in one ClickHouse insert, then clears the re-index campaign.

A single SQL query then enumerates all `v<N>_*` objects in `system.tables` whose version falls
outside a keep-set computed in the same query (active + most recently retired within
