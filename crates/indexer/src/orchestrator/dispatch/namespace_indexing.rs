//! Publishes per-namespace SDLC indexing requests.
...
/// The single publisher of `NamespaceIndexingRequest`. Shared by the scheduled
/// change dispatcher and the enabled-namespaces CDC route so the publish path
/// (validation, envelope, per-subject dedup accounting) lives in one place.
...
impl NamespaceIndexingDispatch {
    ...
    /// Publishes a `NamespaceIndexingRequest` for each (namespace_id, path). The
    /// full SDLC entity set is re-indexed (`targets` empty); per-subject NATS
    /// dedup collapses requests already in-flight for a namespace.
