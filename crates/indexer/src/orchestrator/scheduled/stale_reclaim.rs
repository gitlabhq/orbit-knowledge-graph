...
const CURSOR_KEY_PREFIX: &str = "maintenance.stale_reclaim.";
...
impl StaleReclaim {
    ...
    async fn execute(&self, table: &str, sql: &str) -> Result<(), TaskError> {
        ...
        info!(
            ...
            "stale reclaim statement finished"
        );
        Ok(())
    }
    ...
    async fn prepare(&self) -> Result<bool, TaskError> {
        ...
        let names = self
            ...
            .chain(std::iter::once(self.code_checkpoint_table.as_str()));
        for table in names {
            self.graph
                .query(&enable_block_columns_statement(table))
                .execute()
                .await
                .map_err(TaskError::new)?;
        }
        ...
        for table in &self.tables {
            if self.count(&foreign_block_numbers_sql(&table.name)).await? > 0 {
                warn!(
                    table = table.name,
                    "parts carry block numbers from another table; skipping stale reclaim until the table is renumbered"
                );
...
fn enable_block_columns_statement(table: &str) -> String {
    format!(
...
fn code_scopes_sql(
    ...
    format!(
        "SELECT {CODE_SCOPE}, bound FROM (\
           SELECT {CODE_SCOPE}, max(indexed_at) AS bound FROM {checkpoint_table} \
           WHERE NOT _deleted AND ({CODE_SCOPE}) IN ({changed}) GROUP BY {CODE_SCOPE}) \
         WHERE ({CODE_SCOPE}, bound) IN (\
           SELECT traversal_path, project_id, name, _version FROM {branch_table} \
...
impl ScheduledTask for StaleReclaim {
    ...
    async fn run(&self) -> Result<(), TaskError> {
        ...
        info!(candidates, failed, "stale reclaim pass complete");
        if failed > 0 {
...
mod tests {
    ...
    #[test]
    fn code_scopes_require_a_branch_row_at_the_checkpoint_version() {
        let sql = code_scopes_sql("cp", "br", Some(17), None);
        assert_eq!(
            sql,
            "SELECT traversal_path, project_id, branch, bound FROM (\
             ...
             WHERE (traversal_path, project_id, branch, bound) IN (\
               SELECT traversal_path, project_id, name, _version FROM br \
