//! SDLC indexing pipeline: [`Extractor`] → [`Transformer`] → [`Loader`], wired by
//! [`Pipeline::run`] as a producer task streaming to a consumer over a bounded
//! read-ahead channel.
...
/// Read-ahead window, in transformed blocks. Sized to overlap the next page's
/// read with the current page's writes (cloud throughput parity) while bounding
/// peak memory to a few blocks; benchmarks show parity at 8 and no gain beyond.
...
impl Pipeline {
    ...
    /// The pipeline's only concurrent step: the producer (extract + transform)
    /// streams to the consumer (load) through a bounded read-ahead channel, so
    /// the next page's read overlaps the current page's writes.
    async fn run(
        &self,
        context: &PipelineContext,
        plan: &Plan,
        ...
        let (tx, rx) = mpsc::channel::<WriteCommand>(self.channel_capacity);
...
/// The extension point: each [`Transformation`] turns the source block into one
/// destination table's batch. Transforms are row-wise (`SELECT … FROM
/// source_data`), so a block transforms exactly as a whole page would.
struct Transformer {
    name: String,
...
/// Owned by the spawned task (hence all fields owned) so the next page's read
/// overlaps the current page's writes.
struct Producer {
    extractor: Extractor,
    transformer: Transformer,
    metrics: SdlcMetrics,
...
/// Keeps one streaming insert open across a page's blocks, opened lazily on the
/// first batch so a transform with no rows for the page never opens one.
struct PageWriter<'a> {
