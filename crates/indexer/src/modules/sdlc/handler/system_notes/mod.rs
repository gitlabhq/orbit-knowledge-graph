...
/// End-to-end pass over a batch of extracted notes: parse bodies, build the
/// resolution plan, take a resolver closure to look up targets, and return
/// the resulting edge batch.
///
...
/// and the production handler passes a closure that fans the plan out to
/// the actual `ResolutionPlan` queries (see `resolve::ROUTES_SQL`,
/// `resolve::MERGE_REQUESTS_SQL`, `resolve::WORK_ITEMS_SQL`).
    ...
    R: FnMut(&Reference, &str) -> Option<ResolvedTarget>,
{
    ...
    for n in notes {
        let Some(action) = Action::parse(&n.action) else {
            // Unknown action: log + drop. The production handler emits
            ...
            tracing::debug!(action = %n.action, "system_notes: unknown action, dropping");
            continue;
        };
    ...
    let _ = default_projects; // The default-project mapping drives the resolver
    // closure (provided by the caller); kept on the signature so the
    // production handler doesn't need a churned API once the closure is
    // replaced by the real CH lookups.
...
mod tests {
    ...
    fn process_batch_emits_mentions_edge_for_cross_reference() {
        ...
        let edges = process_batch(&notes, &defaults, |_r, tp| {
            Some(ResolvedTarget {
                id: 456,
    ...
    fn process_batch_emits_user_closed_edge_for_lifecycle_action() {
        ...
        let defaults = DefaultProjectLookup::new();
        let edges = process_batch(&notes, &defaults, |_, _| None);
        assert_eq!(edges.len(), 1);
