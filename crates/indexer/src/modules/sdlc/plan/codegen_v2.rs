//! Parallel codegen using llqm for both extract and transform queries.
//!
//! Produces the same SQL as the existing `lower.rs` + `codegen.rs` pipeline,
//! but via llqm's `PlanBuilder → Substrait → emit_clickhouse_sql` path.
//!
//! This module does NOT replace the existing pipeline — both coexist.
...
///
/// Mirrors `lower::lower_extract_plan` + `ExtractQuery::to_sql` from the
/// existing pipeline.
...
///
/// Mirrors `lower::lower_node_transform` + `codegen::emit_sql`.
...
///
/// Mirrors `lower::lower_fk_edge_transform` + `codegen::emit_sql`.
