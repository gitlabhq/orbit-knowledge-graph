...
#[derive(Clone)]
pub(super) struct SdlcMetrics {
    pipeline_duration: Histogram<f64>,
    pipeline_rows_processed: Counter<u64>,
    pipeline_edges_processed: Counter<u64>,
    pipeline_batches_processed: Counter<u64>,
    pipeline_errors: Counter<u64>,
    handler_duration: Histogram<f64>,
    datalake_query_duration: Histogram<f64>,
    transform_duration: Histogram<f64>,
    watermark_lag: Gauge<f64>,
}
...
impl SdlcMetrics {
    ...
    pub fn record_pipeline_error(&self, entity: &str, error_kind: &str) {
    ...
    pub fn record_datalake_query_duration(&self, entity: &str, duration: f64) {
        self.datalake_query_duration
            .record(duration, &[KeyValue::new("entity", entity.to_owned())]);
    }

    pub fn record_transform_duration(&self, entity: &str, duration: f64) {
        self.transform_duration
            .record(duration, &[KeyValue::new("entity", entity.to_owned())]);
    }

    pub fn record_handler_duration(&self, handler: &str, duration: f64) {
        self.handler_duration
            .record(duration, &[KeyValue::new("handler", handler.to_owned())]);
    }

    pub fn record_watermark_lag(&self, entity: &str, watermark: &DateTime<Utc>) {
        let lag = Utc::now()
