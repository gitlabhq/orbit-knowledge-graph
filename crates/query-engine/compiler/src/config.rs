...
use crate::passes::{
    channel, check, codegen, cursor, enforce, hydrate, lower, normalize, partition, plan, restrict,
...
compiler_pipeline_macros::define_compiler_ctx! {
    ...
    pipelines {
        clickhouse {
            ...
            phases: [validate, normalize, restrict, plan, lower, enforce, security, channel, partition, cursor, check, hydrate_plan, settings, codegen]
        }
        ch_hydration {
            env: [ontology, security_ctx]
...
fn channel(ctx: &mut impl CompilerCtx) -> Result<()> {
    let security_ctx = ctx.security_ctx().clone();
    let ontology = ctx.ontology().clone();
    let mut node = require(ctx.take_node(), "node")?;
    channel::apply_channel_context(&mut node, &security_ctx, &ontology)?;
    ctx.set_node(node);
    Ok(())
}
