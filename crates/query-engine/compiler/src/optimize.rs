...
fn apply_sip_prefilter(q: &mut Query, input: &Input, ctx: &SecurityContext) {
    ...
    for (i, rel) in input.relationships.iter().enumerate() {
        ...
        if let (Some(from), None) = (&from_cte, &to_cte)
            && let Some(cte) =
                build_cascade_for_node(input, &rel.to, end_col, start_col, from, &rel.types)
        {
            let name = format!("_cascade_{}", rel.to);
            q.ctes.push(Cte::new(&name, cte));
            node_ctes.insert(rel.to.clone(), name);
        }
        if let (None, Some(to)) = (&from_cte, &to_cte)
            && let Some(cte) =
                build_cascade_for_node(input, &rel.from, start_col, end_col, to, &rel.types)
        {
