...
fn node_requires_path_scoping(input: &Input, node_id: &str) -> bool {
    input
        .nodes
        .iter()
        .find(|node| node.id == node_id)
        .is_some_and(|node| node.has_traversal_path)
        && input.nodes.iter().any(|node| !node.node_ids.is_empty())
        && input.relationships.iter().any(|rel| {
            (rel.from == node_id
                && input
                    .nodes
                    .iter()
...
fn apply_sip_prefilter(q: &mut Query, input: &Input, ctx: &SecurityContext) {
    ...
    let cte_select = vec![SelectExpr::new(
...
fn build_cascade_for_node(
    ...
    if parent.has_path_tuple && parent.namespace_local && node.has_traversal_path {
...
fn cascade_node_filter_ctes(q: &mut Query, input: &Input) {
    ...
    while changed {
        ...
        for rel in &input.relationships {
            ...
            if rel.namespace_local
                && target.is_some_and(|n| node_requires_path_scoping(input, &n.id))
            {
                select.push(SelectExpr::new(
            ...
            if let Some(cte) = q.ctes.iter_mut().find(|c| c.name == target_nf) {
                let filter = if rel.namespace_local
                    && target.is_some_and(|n| node_requires_path_scoping(input, &n.id))
                {
                    Expr::InSubquery {
...
mod tests {
    ...
    #[test]
    fn cascade_cte_keeps_id_path_for_mixed_scope_target() {
        use crate::input::{Direction, InputNode, InputRelationship};
        ...
        let mut q = Query {
            ctes: vec![Cte::new(
                ...
                Query {
                    select: vec![SelectExpr::new(
                        Expr::col("p", DEFAULT_PRIMARY_KEY),
                        DEFAULT_PRIMARY_KEY,
                    )],
        ...
        let cascade = q
            .ctes
            .iter()
            .find(|cte| cte.name == "_cascade_mr")
            .expect("expected cascade CTE for merge request");
        assert!(
            cascade
                .query
                .select
