    ...
    fn fmt_sexpr(&self, indent: usize) -> String {
        ...
        match self {
            ...
            PhysOp::Filter { input, predicates } => {
                let preds = predicates.iter().map(|p| p.to_sexpr()).collect::<Vec<_>>().join(" ");
                format!("{pad}(Filter [{preds}]\n{})", input.fmt_sexpr(indent + 1))
            }
            PhysOp::Project { input, columns } => {
                let cols = columns.iter().map(|c| c.to_sexpr()).collect::<Vec<_>>().join(" ");
                format!("{pad}(Project [{cols}]\n{})", input.fmt_sexpr(indent + 1))
            }
            PhysOp::Join { left, right, on, kind } => {
                let k = match kind {
                ...
                let on_str = if on.left.1.is_empty() && on.right.1.is_empty() {
                    ...
                    format!("{}.{} = {}.{}", on.left.0, on.left.1, on.right.0, on.right.1)
                };
                format!("{pad}(Join {k} ({on_str})\n{}\n{})", left.fmt_sexpr(indent + 1), right.fmt_sexpr(indent + 1))
            }
            PhysOp::Aggregate { input, group_by, metrics } => {
                let gk = group_by.iter().map(|g| g.to_sexpr()).collect::<Vec<_>>().join(" ");
                let ms = metrics.iter().map(|m| m.to_sexpr()).collect::<Vec<_>>().join(" ");
                format!("{pad}(Agg [group: {gk}] [metrics: {ms}]\n{})", input.fmt_sexpr(indent + 1))
            }
            PhysOp::Union { arms } => {
            ...
            PhysOp::Sort { input, keys } => {
                let ks = keys.iter().map(|k| k.to_sexpr()).collect::<Vec<_>>().join(" ");
                format!("{pad}(Sort [{ks}]\n{})", input.fmt_sexpr(indent + 1))
            }
    ...
    fn to_sexpr(&self) -> String {
        match self {
            ...
            Predicate::In { column, values } => {
                let vs = values.iter().map(|v| v.to_sexpr()).collect::<Vec<_>>().join(",");
                format!("{column}∈[{vs}]")
            }
            ...
            Predicate::Func { name, column, value } => format!("{name}({column},{value})", value = value.to_sexpr()),
...
impl Value {
    fn to_sexpr(&self) -> String {
        match self {
            ...
            Value::Strs(ss) => {
                let items = ss.iter().map(|s| format!("\"{s}\"")).collect::<Vec<_>>().join(",");
                format!("[{items}]")
            }
        }
    }
    ...
    fn to_sexpr(&self) -> String {
        match self {
            ProjectedColumn::Ref { column, alias } => {
                if column == alias { column.clone() } else { format!("{column}:{alias}") }
            }
    ...
    fn to_sexpr(&self) -> String {
        match self {
            ...
            ColumnExpr::Array(items) => {
                let inner = items.iter().map(|i| i.to_sexpr()).collect::<Vec<_>>().join(" ");
                format!("[{inner}]")
            }
...
impl GroupKey {
    fn to_sexpr(&self) -> String {
        let trunc = self.truncate.map(|t| format!("/{}", t.ch_function())).unwrap_or_default();
