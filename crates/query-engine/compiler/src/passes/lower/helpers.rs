...
/// JOIN a node's latest-row scan into the FROM tree.
///
/// When `sort_key` is provided, uses `LIMIT 1 BY` with PK-prefixed
/// ORDER BY instead of FINAL for dedup. This avoids the ReplacingSorted
/// merge which reads all columns.
