...
// Re-export types from v1 enforce so the pipeline state sees the same types.
pub use super::super::enforce::{EdgeMeta, RedactionNode, ResultContext};
...
pub fn enforce_return(node: &mut Node, input: &Input) -> Result<ResultContext> {
    ...
    for (entity, config) in &input.entity_auth {
        ctx.add_entity_auth(entity, config.clone());
    }
