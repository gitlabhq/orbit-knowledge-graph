...
impl<'a> Validator<'a> {
    ...
    /// ADR 013: reject any referenced entity absent from the channel-scoped
    /// graph. A gated entity has no node there, so it reads as an unknown
    /// identifier through the normal path, no separate "restricted" error and
    /// no way for a gated table to reach a `FROM` clause. `None` (full graph) is
    /// a no-op because every declared entity is present.
    pub fn check_entities_present(
        &self,
        input: &Input,
        ...
        for node in &input.nodes {
            if let Some(entity) = node.entity.as_deref()
                && graph.node_template(entity).is_none()
            {
...
mod tests {
    ...
    #[test]
    fn gated_entity_is_absent_from_scoped_graph_so_reads_as_unknown() {
        let ontology = test_ontology()
        ...
        Validator::new(&ontology)
            .check_entities_present(&input, Some(ontology::Channel::CoreFeature))
            .unwrap();

        let err = Validator::new(&ontology)
            .check_entities_present(&input, Some(ontology::Channel::ExternalAgent))
            .unwrap_err();
        assert!(
    ...
    #[test]
    fn entities_present_accepts_entity_visible_on_channel() {
