...
impl QuotaCache {
    ...
    pub(crate) async fn check(&self, key: CacheKey) -> QuotaDecision {
        // moka's `try_get_with` coalesces concurrent callers that miss on the
        // same key: exactly one closure runs, the rest await the result. This
        // is the single-flight requirement from the issue.
        //
        // FailOpen returns Err so it never populates the cache; we short-circuit
        // to Allow below.
...
mod tests {
    ...
    async fn caches_deny_decisions() {
        ...
        let key = key_with("denied");
        for _ in 0..3 {
            assert_eq!(
                cache.check(key.clone()).await,
