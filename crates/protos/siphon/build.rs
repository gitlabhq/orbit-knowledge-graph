...
fn regenerate_protos() {
    println!("cargo:rerun-if-env-changed=SIPHON_PROTO_ROOT");

    let Ok(root) = std::env::var("SIPHON_PROTO_ROOT").map(|p| expand_tilde(&p)) else {
        println!("cargo:warning=SIPHON_PROTO_ROOT not set, skipping proto regeneration");
        return;
    };
    let proto_dir = format!("{root}/pb");

    if !std::path::Path::new(&proto_dir).exists() {
        println!(
            "cargo:warning=SIPHON_PROTO_ROOT path does not exist ({proto_dir}), skipping proto regeneration"
        );
        return;
    }
    ...
    let protos = [
        format!("{proto_dir}/siphon/v1/toast_marker.proto"),
        format!("{proto_dir}/siphon/v1/int16_array.proto"),
        format!("{proto_dir}/siphon/v1/int64_array.proto"),
    ...
    tonic_prost_build::configure()
        ...
        .expect("Failed to compile Siphon protos");
