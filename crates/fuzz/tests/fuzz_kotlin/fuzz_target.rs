...
use code_graph::v2::langs::generic::kotlin::KotlinDsl;
...
fn spec() -> &'static code_graph::v2::dsl::types::LanguageSpec {
    ...
    SPEC.get_or_init(KotlinDsl::spec)
}

fn main() {
    let tracer = Tracer::new(false);
    check!().for_each(|input: &[u8]| {
        let _ = spec().parse_full_collect(input, "fuzz.kt", Language::Kotlin, &tracer, None);
    });
}
