#!/bin/bash
# RSS profiler for tree-dsl --bench. Adapted from gkg's average_rss.sh.
# Usage: ./rss_bench.sh <directory> [sampling_interval_seconds]

if [ -z "$1" ]; then
  echo '{"error": "Usage: ./rss_bench.sh <path> [sampling_interval_seconds]"}'
  echo '{"example": "./rss_bench.sh /tmp/sentry 0.025"}'
  exit 1
fi
...
echo "Monitoring tree-dsl bench (pid=$PROCESS_PID, path=$1, sampling_interval=$SAMPLING_INTERVAL s)..." >&2
...
if [ $num_samples -gt 0 ]; then
  ...
  # Print bench output
  cat /tmp/tree_dsl_bench_stderr.txt >&2
