...
# 2. Concurrent readers return identical results
...
# 3. Reader succeeds while writer holds lock
...
# 4. Concurrent writers both succeed (retry backoff)
result=$(run_concurrent_writers "$REPO" 2)
[ "$result" != "none" ] \
    && add "concurrent_writers" true "$result succeeded" \
    || add "concurrent_writers" false "both failed"

# 5. Data intact after concurrent writes
orbit_query "$Q_FILES_SIMPLE" "$TMP/integrity.json"
assert_has "data_integrity" "$TMP/integrity.json" "true"

# 6. Sequential reads are consistent
