...
pub(super) async fn search_returns_correct_user_properties(ctx: &TestContext) {
    let resp = run_query(
        ...
        r#"{
            "query_type": "search",
            "node": {"id": "u", "entity": "User", "node_ids": [1, 2, 3, 4, 5, 6], "columns": ["username", "name", "state", "user_type"]},
...
pub(super) async fn search_returns_correct_group_full_path(ctx: &TestContext) {
    let resp = run_query(
        ...
        r#"{
            "query_type": "search",
            "node": {"id": "g", "entity": "Group", "node_ids": [100, 101, 102, 200, 300], "columns": ["name", "full_path"]},
...
pub(super) async fn search_returns_correct_project_full_path(ctx: &TestContext) {
    let resp = run_query(
        ...
        r#"{
            "query_type": "search",
            "node": {"id": "p", "entity": "Project", "node_ids": [1000, 1001, 1002, 1003, 1004], "columns": ["name", "full_path"]},
...
pub(super) async fn search_default_columns_include_full_path(ctx: &TestContext) {
    let resp = run_query(
        ...
        r#"{
            "query_type": "search",
            "node": {"id": "g", "entity": "Group", "node_ids": [100, 101, 102, 200, 300]},
            "limit": 10
        }"#,
        &allow_all(),
    )
...
pub(super) async fn search_datetime_columns_serialize_as_strings(ctx: &TestContext) {
    let resp = run_query(
        ...
        r#"{
            "query_type": "search",
            "node": {"id": "n", "entity": "Note", "node_ids": [3000, 3001, 3002, 3003], "columns": ["note", "created_at"],
                     "node_ids": [3000, 3001]},
            "limit": 10
        }"#,
        &allow_all(),
