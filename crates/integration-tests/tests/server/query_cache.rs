...
fn load_ontology() -> Arc<Ontology> {
    Arc::new(Ontology::load_embedded().expect("embedded ontology should load"))
}
...
/// Build a QueryPipelineContext with a compiled query and server extensions.
fn build_context(
...
async fn cached_executor_miss_then_hit() {
    ...
    // Compile query with app cache enabled
    let ontology = load_ontology();
    let security_ctx = test_security_context();
    let mut compiled =
        compile(SIMPLE_TRAVERSAL, &ontology, &security_ctx).expect("compilation should succeed");
    compiled.base.query_config.app_cache_enabled = Some(true);
    compiled.base.query_config.app_cache_ttl = Some(300);
    let compiled = Arc::new(compiled);

    let client = Arc::new(ch_ctx.create_client());

    // --- CALL 1: cache miss → ClickHouse → result stored in KV ---
    ...
    // Wait for the fire-and-forget KV write to complete
    ...
    // Verify ClickHouse is empty: a direct query should return 0 rows
...
async fn cached_executor_bypass_when_disabled() {
    ...
    // Compile query with cache DISABLED (default)
    let ontology = load_ontology();
    let security_ctx = test_security_context();
    let compiled =
        compile(SIMPLE_TRAVERSAL, &ontology, &security_ctx).expect("compilation should succeed");
    assert!(
        compiled.base.query_config.app_cache_enabled.is_none()
            || compiled.base.query_config.app_cache_enabled == Some(false),
    ...
    // Execute - should hit ClickHouse directly, not cache
    ...
    // Execute again - should hit ClickHouse (empty), NOT cache
    let mut ctx2 = build_context(
        &ontology,
        Arc::clone(&compiled),
        Arc::clone(&client),
...
async fn cached_executor_works_without_nats() {
    ...
    // Build context WITHOUT NatsBroker - CachedExecutor should fall through
    let mut server_extensions = TypeMap::default();
    server_extensions.insert(client);
    server_extensions.insert(gkg_server_config::ProfilingConfig::default());
