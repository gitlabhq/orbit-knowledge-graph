...
use compiler::{Frontend, Node, PreparedStatement, QueryError, compile, prepare};
...
fn orbit_query_virtual_filter_equality_hydration_parity() {
    ...
    for (json, query) in [
        ...
        let PreparedStatement::Query(prepared) =
            prepare(query, Frontend::Gql, &ontology, &test_ctx(), All).unwrap()
        else {
...
fn prepare_dispatches_queries_and_scoped_schema() {
    ...
    for (raw, fe) in [(json, Frontend::JsonDsl), (query, Frontend::Gql)] {
        let PreparedStatement::Query(prepared) = prepare(raw, fe, &ontology, &ctx, All).unwrap()
        else {
            panic!("expected compiled query: {raw}");
        };
        assert_eq!(prepared.base.sql, compiled.base.sql);
        assert_eq!(prepared.base.params, compiled.base.params);
        assert_eq!(prepared.hydration, compiled.hydration);
    ...
    for (scope, call, node) in [
        ...
        let PreparedStatement::Schema(response) =
            prepare(call, Frontend::Gql, &ontology, &ctx, scope).unwrap()
        else {
...
fn prepare_uses_supplied_ontology_and_rejects_wildcards() {
    ...
    let ctx = test_ctx();
    let PreparedStatement::Schema(response) = prepare(
        "CALL db.schema('Custom')",
        Frontend::Gql,
        &ontology,
        &ctx,
        All,
    ...
    for (scope, name) in [(All, "MergeRequest"), (All, "*"), (Local, "Custom")] {
        ...
        let error = prepare(&call, Frontend::Gql, &ontology, &ctx, scope).unwrap_err();
...
fn prepare_rejects_invalid_calls_and_preserves_query_only_apis() {
    ...
    for call in [
        ...
        let error = prepare(call, Frontend::Gql, &ontology, &ctx, All).unwrap_err();
    ...
    for call in ["CALL db.schema()", "CALL db.schema('MergeRequest')"] {
        assert!(matches!(
            gql::parse(call),
            Ok(compiler::passes::frontend::Statement::Schema(_))
        ));
        for error in [
            compile(call, Frontend::Gql, &ontology, &ctx).unwrap_err(),
            compile(call, Frontend::JsonDsl, &ontology, &ctx).unwrap_err(),
            prepare(call, Frontend::JsonDsl, &ontology, &ctx, All).unwrap_err(),
        ] {
            assert!(error.is_client_safe(), "{call}: {error}");
        }
    }
...
fn prepare_enforces_existing_input_bounds() {
    ...
    for raw in ["CALL db.schema()", "MATCH (n:User {id: 1}) RETURN n"] {
        ...
        assert!(prepare(&at_limit, Frontend::Gql, &ontology, &ctx, All).is_ok());
        let error = prepare(&(at_limit + " "), Frontend::Gql, &ontology, &ctx, All).unwrap_err();
        assert!(matches!(error, QueryError::LimitExceeded(_)), "{error}");
        let commented = format!("{raw} /*{}*/", "(".repeat(64));
        assert!(prepare(&commented, Frontend::Gql, &ontology, &ctx, All).is_ok());
    }
    let nested = format!("CALL db.schema({})", "(".repeat(32));
    let error = prepare(&nested, Frontend::Gql, &ontology, &ctx, All).unwrap_err();
    assert!(error.is_client_safe());
    assert!(error.to_string().contains("nesting is too deep"), "{error}");
}
