...
fn compile_to_ast_works() {
    let opencypher = "MATCH (u:User {id: 1}) RETURN u.username LIMIT 10";
    let json = r#"{
    ...
    compile_pair(json, opencypher, &test_ontology(), &test_ctx()).unwrap();
...
#[test]
fn traversal_query() {
    let opencypher = "MATCH (n:Note {confidential: true})<-[:AUTHORED]-(u:User) RETURN n.confidential, u.username ORDER BY n.created_at DESC LIMIT 25";
    let json = r#"{
        "query_type": "traversal",
        "nodes": [
    ...
    let result = compile_pair(json, opencypher, &test_ontology(), &test_ctx()).unwrap();
...
fn bool_filter_value_is_preserved() {
    let opencypher = "MATCH (n:Note {confidential: true}) RETURN n.confidential LIMIT 5";
    let json = r#"{
        "query_type": "traversal",
        "nodes": [{
            "id": "n",
...
fn aggregation_query() {
    let opencypher = "MATCH (n:Note {id: 1})<-[:AUTHORED]-(u:User) RETURN u{.id, .username}, count(n) AS note_count LIMIT 10";
    let json = r#"{
...
fn group_by_property_truncate_month_wraps_column() {
    let opencypher = "MATCH (u:Note {confidential: false}) RETURN date_trunc('month', u.created_at), count(u) AS n LIMIT 50";
    let json = r#"{
        "query_type": "aggregation",
        "nodes": [
...
fn group_by_property_truncate_all_units_compile() {
    for unit in ["minute", "hour", "day", "week", "month", "quarter", "year"] {
        ...
        let result = compile_pair(&json, &opencypher, &test_ontology(), &test_ctx())
...
fn group_by_truncate_minute_without_selectivity_rejected() {
    ...
    let err = compile_pair(json, opencypher, &test_ontology(), &test_ctx()).unwrap_err();
    let msg = format!("{err:?}");
    assert!(
...
fn group_by_truncate_minute_with_node_ids_accepted() {
    let opencypher = "MATCH (u:Note) WHERE u.id IN [1, 2] RETURN date_trunc('minute', u.created_at), count(u) AS n LIMIT 10";
    let json = r#"{
        "query_type": "aggregation",
        "nodes": [
...
fn group_by_truncate_hour_with_property_filter_accepted() {
    let opencypher = "MATCH (u:Note) WHERE u.created_at >= '2026-04-01T00:00:00Z' RETURN date_trunc('hour', u.created_at), count(u) AS n LIMIT 50";
    let json = r#"{
        "query_type": "aggregation",
        "nodes": [
...
fn group_by_truncate_custom_alias_preserved() {
    let opencypher = "MATCH (u:Note {id: 1}) RETURN date_trunc('month', u.created_at) AS bucket, count(u) AS n LIMIT 10";
    let json = r#"{
        "query_type": "aggregation",
        "nodes": [
            {"id": "u", "entity": "Note", "node_ids": [1]}
...
fn path_finding_query() {
    let opencypher = "MATCH p = shortestPath((start:Project {id: 100})-[:CONTAINS*1..3]->(`end`:Project {id: 200})) RETURN p";
    let json = r#"{
...
fn path_finding_depth_control() {
    ...
    let shallow_opencypher = "MATCH p = shortestPath((start:Project {id: 1})-[:CONTAINS|MEMBER_OF*1]->(`end`:Project {id: 2})) RETURN p";
    let deep_opencypher = "MATCH p = shortestPath((start:Project {id: 1})-[:CONTAINS|MEMBER_OF*1..3]->(`end`:Project {id: 2})) RETURN p";
    let shallow_sql = compile_pair(shallow, shallow_opencypher, &test_ontology(), &test_ctx())
        .unwrap()
        .base
        .render();
    let deep_sql = compile_pair(deep, deep_opencypher, &test_ontology(), &test_ctx())
        .unwrap()
        .base
        .render();
...
#[test]
fn neighbors_query() {
    let opencypher = "MATCH (u:User {id: 100})--(n) RETURN n";
    let json = r#"{
...
#[test]
fn filter_operators() {
    let opencypher = "MATCH (u:User) WHERE u.created_at >= '2024-01-01' AND u.state IN ['active', 'blocked'] AND u.username CONTAINS 'admin' RETURN u.username, u.state, u.created_at LIMIT 30";
    let json = r#"{
        "query_type": "traversal",
        "nodes": [{
            "id": "u",
...
fn multi_table_single_type_routes_to_default() {
    let opencypher = "MATCH (u:User {id: 1})-[:AUTHORED]->(p:Project) RETURN u, p LIMIT 25";
    let json = r#"{
        "query_type": "traversal",
        "nodes": [
    ...
    let result = compile_pair(json, opencypher, &multi_table_ontology(), &test_ctx()).unwrap();
    let rendered = result.base.render();
    assert!(
...
fn multi_table_code_edge_routes_to_code_table() {
    let opencypher = "MATCH (f:File {id: 1})-[:DEFINES]->(d:Definition) RETURN f, d LIMIT 25";
    let json = r#"{
        "query_type": "traversal",
        "nodes": [
...
fn multi_table_wildcard_scans_all_tables() {
    let opencypher = "MATCH (u:User {id: 1})-->(p:Project) RETURN u, p LIMIT 25";
...
fn multi_table_mixed_types_scans_both_tables() {
    let opencypher = "MATCH (u:User {id: 1})-[:AUTHORED|DEFINES]->(p:Project) RETURN u, p LIMIT 25";
...
fn multi_table_path_finding_scans_all_tables() {
    let opencypher = "MATCH path = shortestPath((start:User {id: 1})-[:CONTAINS|DEFINES*1..3]->(`end`:Definition {id: 100})) RETURN path";
    let json = r#"{
        "query_type": "path_finding",
        "nodes": [
...
fn neighbors_non_default_pk_with_non_denorm_filter_no_alias_clash() {
    let opencypher = "MATCH (f:File)--(n) WHERE f.path CONTAINS 'labkit' RETURN n";
    use ontology::DataType;
    ...
    let result = compile_pair(json, opencypher, &ontology, &test_ctx()).unwrap();
    let rendered = result.base.render();
...
fn multi_table_neighbors_scans_all_tables() {
    let opencypher = "MATCH (p:Project {id: 1})--(n) RETURN n";
    let json = r#"{
        "query_type": "neighbors",
...
fn render_scoped(json: &str, opencypher: &str) -> String {
    compile_pair(json, opencypher, &embedded_ontology(), &scoped_ctx())
        .unwrap()
        .base
        .render()
}
...
fn scoped_traversal_injects_tight_prefix() {
    ...
    assert!(render_scoped(json, opencypher).contains(SCOPED_PREFIX));
}

#[test]
fn scoped_aggregation_injects_tight_prefix() {
    let opencypher = "MATCH (wi:WorkItem)-[:IN_PROJECT]->(p:Project) WHERE p.id = 1 RETURN p, count(wi) AS c LIMIT 100";
    let json = r#"{
        "query_type": "aggregation",
        "nodes": [
            {"id": "wi", "entity": "WorkItem", "columns": ["id"]},
...
fn cross_namespace_related_to_edge_stays_unscoped() {
    let opencypher = "MATCH (p:Project)<-[:IN_PROJECT]-(wi:WorkItem)-[:RELATED_TO]->(rel:WorkItem) WHERE p.id = 1 RETURN p, wi.id, rel.id, rel.title LIMIT 100";
    let json = r#"{
        "query_type": "traversal",
        "nodes": [
            {"id": "p", "entity": "Project", "filters": {"id": {"eq": 1}}},
    ...
    let compiled = compile_pair(json, opencypher, &ontology, &scoped_ctx()).unwrap();
...
#[test]
fn opencypher_bounded_hops_and_relationship_filters() {
    let cases = [
        (
    ...
    for (json, opencypher) in cases {
        compile_pair(json, opencypher, &embedded_ontology(), &test_ctx()).unwrap();
    }
}

#[test]
fn opencypher_id_selectors_preserve_additional_predicates() {
    let cases = [
        (
...
#[test]
fn opencypher_aggregate_functions_property_groups_and_ordering() {
    let json = r#"{
        "query_type":"aggregation",
    ...
    let opencypher = "MATCH (u:Note) WHERE u.id IN [1, 2] RETURN u.confidential AS confidential, count(u) AS n, sum(u.id), avg(u.id), min(u.created_at), max(u.created_at) ORDER BY n DESC LIMIT 10";
    compile_pair(json, opencypher, &test_ontology(), &test_ctx()).unwrap();
}

#[test]
fn opencypher_null_predicates_and_all_columns() {
...
#[test]
fn opencypher_parameters_are_bound_as_values() {
    ...
    .to_string();
    let opencypher = "MATCH (u:User) WHERE u.username = $name RETURN u LIMIT $limit";
    let parameters = opencypher::Parameters::from([
...
#[test]
fn opencypher_m23_literals_and_comments() {
    let cases = [
        (
...
#[test]
fn opencypher_rejects_unsupported_syntax_and_shapes() {
    let queries = [
        "CREATE (u:User)",
...
#[test]
fn opencypher_preserves_ontology_validation_and_security() {
    let cases = [
        (
    ...
    for (json, opencypher) in cases {
        compile_pair(json, opencypher, &test_ontology(), &test_ctx()).unwrap_err();
    }
    let json = r#"{"query_type":"traversal","nodes":[{"id":"p","entity":"Project","filters":{"traversal_path":{"starts_with":"1/"}}}]}"#;
    let opencypher = "MATCH (p:Project) WHERE p.traversal_path STARTS WITH '1/' RETURN p";
    let context = compiler::SecurityContext::new(1, vec!["1/24/".into()]).unwrap();
    let error = compile_pair(json, opencypher, &embedded_ontology(), &context).unwrap_err();
    assert!(matches!(error, QueryError::Authorization(_)));
}

#[test]
fn opencypher_bounds_input_before_recursive_parsing() {
    let nested = format!(
        "MATCH (u:User) WHERE {}u.id = 1{} RETURN u",
        "(".repeat(40),
        ")".repeat(40)
    ...
    for query in [nested, oversized] {
        assert!(opencypher::parse(&query, &opencypher::Parameters::new()).is_err());
    }
    for query in [
