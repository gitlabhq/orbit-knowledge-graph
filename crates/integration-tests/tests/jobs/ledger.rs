...
impl Scenario {
    ...
    async fn open_with_code_jobs(&self, keys: &[&str]) -> Vec<JobRef> {
        self.ledger()
            .open_campaign(&self.campaign, &Self::phases())
            .await
            .unwrap();
...
async fn registering_the_same_jobs_twice_does_not_inflate_counts() {
    ...
    assert_eq!(
        counts(&scenario.summary().await, &CODE, JobState::Pending),
        2
    );
}
...
async fn transitions_move_a_job_through_its_lifecycle() {
    ...
    scenario
        .record(&jobs[0], dispatch, 0, JobState::Queued, now)
        .await;
    scenario
        .record(
            &jobs[0],
...
async fn a_late_lower_ranked_row_cannot_regress_the_same_attempt() {
    ...
    scenario
        .record(&jobs[0], dispatch, 1, JobState::Succeeded, now)
        .await;
    scenario
        .record(
            &jobs[0],
...
async fn a_late_row_from_an_older_attempt_cannot_change_the_newer_attempt() {
    ...
    scenario
        .record(&jobs[0], dispatch, 2, JobState::Succeeded, now)
        .await;
    scenario
        .record(
            &jobs[0],
...
async fn a_newer_dispatch_supersedes_an_older_one() {
    ...
    scenario
        .record(&jobs[0], Uuid::new_v4(), 1, JobState::Failed, now)
        .await;
    scenario
        .record(
            &jobs[0],
...
async fn campaign_completes_when_discovery_is_closed_and_every_job_is_terminal() {
    ...
    scenario
        .record(&jobs[0], Uuid::new_v4(), 1, JobState::Succeeded, now)
        .await;
    scenario
        .record(&jobs[1], Uuid::new_v4(), 1, JobState::Failed, now)
        .await;
    assert!(!scenario.summary().await.is_complete());

    scenario
        .ledger()
        .close_discovery(&scenario.campaign, &phases[0])
        .await
        .unwrap();
    assert!(!scenario.summary().await.is_complete());
    ...
    scenario
        .ledger()
        .close_discovery(&scenario.campaign, &phases[1])
        .await
        .unwrap();
    let summary = scenario.summary().await;
    assert!(summary.is_complete());
...
async fn latest_campaign_returns_the_newest_generation_only() {
    ...
    scenario
        .ledger()
        .open_campaign(&next, &Scenario::phases())
        .await
        .unwrap();
    scenario
        .ledger()
        .register(
            &[code_job(Some(next.clone()), "7")],
            JobState::Pending,
            None,
        )
        .await
...
async fn pending_jobs_returns_only_pending_jobs_up_to_the_limit() {
    ...
    let now = Utc::now();
    scenario
        .record(&jobs[0], Uuid::new_v4(), 0, JobState::Queued, now)
        .await;
    scenario
        .record(&jobs[1], Uuid::new_v4(), 1, JobState::Succeeded, now)
        .await;
...
async fn latest_success_at_scopes_by_namespace_and_path_prefix() {
    let scenario = Scenario::new().await;
    let jobs = scenario.open_with_code_jobs(&["7", "8"]).await;
    let earlier = Utc.with_ymd_and_hms(2026, 9, 21, 13, 0, 0).unwrap();
    ...
    scenario
        .record(&jobs[0], Uuid::new_v4(), 1, JobState::Succeeded, earlier)
        .await;
    scenario
        .record(&jobs[1], Uuid::new_v4(), 1, JobState::Succeeded, later)
        .await;
...
async fn jobs_filter_returns_one_snapshot_per_job_with_its_latest_transition() {
    ...
    let now = Utc::now();
    scenario
        .record(&jobs[0], dispatch, 1, JobState::Running, now)
        .await;
    scenario
        .ledger()
        .record(&JobTransition {
    ...
    let snapshots = scenario.ledger().jobs(&filter).await.unwrap();
    ...
    let pending = scenario.ledger().jobs(&filter).await.unwrap();
...
async fn boot_adds_missing_columns_to_an_existing_unversioned_table() {
    ...
    context
        .execute("CREATE TABLE campaign (kind String, _version UInt64) ENGINE = ReplacingMergeTree(_version) ORDER BY kind")
        .await;
    ...
    let batches = context
        .query("SELECT name FROM system.columns WHERE database = currentDatabase() AND table = 'campaign' ORDER BY position")
        .await;
    let names =
        <String as clickhouse_client::FromArrowColumn>::extract_column(&batches, 0).unwrap();
    for expected in [
        "kind",
        "subject",
        "generation",
