...
// `SIPHON_WATERMARK_COLUMN` and `SIPHON_DELETED_COLUMN` are generated by
// build.rs from `schema.yaml`'s `settings.etl.default_watermark` /
// `default_deleted`, so the ontology YAML is the single source of truth.
include!(concat!(env!("OUT_DIR"), "/siphon_columns.rs"));
...
pub fn validate_ontology_constants(ontology: &crate::Ontology) {
    ...
    assert_eq!(
        ...
        "generated SIPHON_WATERMARK_COLUMN (\"{SIPHON_WATERMARK_COLUMN}\") doesn't match \
         embedded ontology (\"{}\") — rebuild the ontology crate so build.rs regenerates \
         from default_watermark in schema.yaml",
        ontology.default_watermark_column(),
    );
    ...
    assert_eq!(
        ...
        "generated SIPHON_DELETED_COLUMN (\"{SIPHON_DELETED_COLUMN}\") doesn't match \
         embedded ontology (\"{}\") — rebuild the ontology crate so build.rs regenerates \
         from default_deleted in schema.yaml",
        ontology.default_deleted_column(),
    );
}
