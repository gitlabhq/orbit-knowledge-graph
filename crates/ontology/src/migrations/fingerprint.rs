...
# ledger entry. Active-object drift requires `mise schema:snapshot` and does
...
pub struct Fingerprints {
    ...
    #[serde(default)]
    pub active_objects: BTreeMap<String, String>,
}
...
impl Fingerprints {
    ...
    #[must_use]
    pub fn get_active_object_diff_keys_between(&self, base: &Fingerprints) -> BTreeSet<String> {
        get_diff_keys_between(&self.active_objects, &base.active_objects)
    }
...
fn stable_versioned_schema_hash(content: &str) -> String {
    ...
    remove_active_object_settings(&mut value);
    sort_yaml_keys(&mut value);
...
fn remove_active_object_settings(value: &mut serde_yaml::Value) {
    ...
    retain_versioned_insert_trigger_views(settings);
}
...
fn retain_versioned_insert_trigger_views(settings: &mut serde_yaml::Mapping) {
    let key = serde_yaml::Value::String("materialized_views".to_string());
    let Some(serde_yaml::Value::Sequence(views)) = settings.get_mut(&key) else {
        return;
    };
    views.retain(|view| {
        let Some(mapping) = view.as_mapping() else {
            return true;
        };
        let versioned = mapping
            .get(serde_yaml::Value::String("versioned".to_string()))
            .and_then(serde_yaml::Value::as_bool)
            .unwrap_or(false);
        let insert_trigger = mapping
            .get(serde_yaml::Value::String("kind".to_string()))
            .and_then(serde_yaml::Value::as_str)
            == Some("insert_trigger");
        versioned && insert_trigger
    });
    if views.is_empty() {
        settings.remove(&key);
    }
