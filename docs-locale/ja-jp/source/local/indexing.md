...
> [!note]
> Orbit Localは実験的な機能です。GAリリース前に、機能やコマンドの形式が
> 変更される場合があります。
...
複数のリポジトリを同一のDuckDBファイルにインデックスすることができます。各リポジトリは絶対パスによって個別に追跡されます。
...
- 関数、クラス、メソッド、モジュールの定義（開始・終了行番号および完全なソースコンテンツを含む）
...
Orbit Localは、Orbit Remoteと同じ11言語をサポートしており、クロスファイル参照の解決にも対応しています。
