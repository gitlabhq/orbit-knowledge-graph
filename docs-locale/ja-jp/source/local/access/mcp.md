...
Orbit Localは MCPサーバーとして実行でき、Orbit Remoteと同じ2つのツール（`query_graph`、`get_graph_schema`）を公開しますが、GitLabインスタンスではなくローカルのDuckDBグラフを参照します。

> [!note]
> MCPサーバーは実験的な機能です。GAになる前に、機能や設定の形式が変更される場合があります。
...
Orbit Remote（JSON クエリDSLを公開）とは異なり、Orbit LocalはrawのDuckDB SQLを使用します。エージェントはプロパティグラフテーブルに対して直接SQLを組み立てます。

> [!note]
> 計画中の `glab orbit setup` サブコマンドにより、Orbitスキルのインストールとお使いのMCPクライアント向けのMCP設定の書き込みが自動化される予定です。このコマンドがリリースされるまでは、以下に示す手順でMCPクライアントを手動で設定してください。
...
スキーマを確認する：
> 「`get_graph_schema` を使用して、ローカルグラフにどのようなノードタイプがあるか表示してください。」
...
Orbit Localはコードのみをインデックスします：サポートされている11言語すべてにわたるファイル、ディレクトリ、定義、およびインポートされたシンボルです。SDLCデータ（マージリクエスト、パイプライン、ユーザー、脆弱性）はローカルでは利用できません。これらには[Orbit Remote](../../remote/_index.md)が必要です。

## 課金 {#billing}

Orbit LocalはGitLab Creditsを消費しません。すべてのMCPトラフィックはローカルで処理されます。
