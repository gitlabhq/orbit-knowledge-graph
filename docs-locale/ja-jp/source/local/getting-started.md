...
`orbit`バイナリは、ワンラインインストーラーで直接インストールするか、すでに使用している場合はGitLab CLI（`glab`）経由でインストールできます。
...
クエリ言語は3つすべてで同一です。1つで学んだことは他のすべてに直接応用でき、[Orbit Remote](../remote/_index.md)にも適用できます。
...
- AIエージェントに接続する: 手動設定については[MCPで接続する](access/mcp.md)を参照してください（`glab orbit setup`による自動化は予定）
