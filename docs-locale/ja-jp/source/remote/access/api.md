...
結果は、トークンオーナーがGitLabでアクセスできるエンティティに限定されます。

## 課金 {#billing}

APIコールはサブスクリプションのGitLabクレジットを消費します。`POST /api/v4/orbit/query`への各コールにはクレジットが使用されます。その他のエンドポイントは無料です。
...
| `GET` | `/api/v4/orbit/schema` | 現在のスキーマを取得する |
