...
すべてのエンドポイントには、`read_api`スコープを持つGitLabパーソナルアクセストークンが必要です。Bearerトークンとして渡します。
...
APIコールはサブスクリプションのGitLab Creditsを消費します。`POST /api/v4/orbit/query`への各コールはクレジットを使用します。その他のエンドポイントは無料です。
...
クエリを記述する前に、利用可能なエンティティタイプとプロパティを確認するために使用します。
...
MCPクライアントと互換性のある形式で、`query_graph`および`get_graph_schema`のMCPツール定義を返します。
