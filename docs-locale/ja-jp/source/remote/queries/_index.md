...
Orbitクエリは、グラフ操作を記述するJSONオブジェクトです。クエリでは、1種類のオブジェクトのフェッチ、オブジェクト間の関係のトラバース、一致するオブジェクトのカウント、パスの検索、またはノードの隣接要素の取得が可能です。
...
| `Definition` | `content` | 1つのインデックス済み定義のソーステキスト |
...
`HAS_DIFF`を使用してマージリクエストから差分スナップショットに移動し、次に`HAS_FILE`を使用してそれらのスナップショット内のファイルをフェッチします。
...
ソースコードエンティティには`content`を使用します。この例では、パスによってインデックス済みファイルを検索し、rawファイルテキストを返します。
