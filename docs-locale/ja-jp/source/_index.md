...
description: GitLabインスタンスをプロパティグラフとしてクエリします。影響範囲の特定、依存関係のトレース、GitLab単体では回答できないSDLCに関する質問への対応が可能です。
title: GitLab Orbit
---
...
- [GitLab Orbit Remoteを有効にして最初のクエリを実行する](remote/getting-started.md)
- [GitLab Orbit Localでローカルコードグラフを構築する](local/getting-started.md)
- [GitLab Self-ManagedにGitLab Orbitをインストールする](self-managed/getting-started.md)
- [GitLab OrbitスキルでAIコーディングエージェントをセットアップする](ai_coding_agents.md)
