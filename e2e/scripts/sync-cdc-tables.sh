...
#
# Downloads the `siphon-ssot-tables` artifact for the pinned `gitlab.ref` from
# this project's package registry (see `publish-siphon-tables.sh`), filters to
# `database: main` (e2e PG has no ci/sec shards), and runs the siphon
# `schema generate-values` binary inside its docker image to emit two helm
# value fragments:
#
...
#
# `values/siphon.yaml.gotmpl` reads both files at render time.
#
...
PKG_URL="https://gitlab.com/api/v4/projects/${PKG_PROJECT}/packages/generic/siphon-ssot-tables/${GITLAB_REF}/tables.tar.gz"
AUTH_HEADER=()
[[ -n "${CI_JOB_TOKEN:-}" ]] && AUTH_HEADER=(--header "JOB-TOKEN: ${CI_JOB_TOKEN}")
log "fetching $PKG_URL"
curl -sfL ${AUTH_HEADER[@]+"${AUTH_HEADER[@]}"} --retry 3 --retry-delay 2 --connect-timeout 15 --max-time 120 -o "$ARCHIVE" "$PKG_URL" \
  || { echo "no siphon-ssot-tables artifact for $GITLAB_REF; run e2e/scripts/publish-siphon-tables.sh"; exit 1; }
gzip -t "$ARCHIVE" 2>/dev/null || { echo "downloaded artifact is not gzip"; exit 1; }
tar -xzf "$ARCHIVE" -C "$TABLES_RAW"
...
# Source: siphon-ssot-tables package (gitlab-org/gitlab @ ${GITLAB_REF}), filtered to database: main.
