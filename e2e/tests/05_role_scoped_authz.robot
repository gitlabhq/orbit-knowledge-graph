...
Query Vulnerability Counts As Victim
    ...
    ${query}=    Evaluate
    ...    {"query_type": "aggregation", "nodes": [{"id": "p", "entity": "Project", "columns": ["name"], "node_ids": $project_ids}, {"id": "v", "entity": "Vulnerability"}], "relationships": [{"type": "IN_PROJECT", "from": "v", "to": "p"}], "aggregations": [{"function": "count", "target": "v", "alias": "vuln_count"}], "group_by": ["p"], "limit": 20}
...
Query Vulnerability Counts For Project As Victim
    ...
    ${query}=    Evaluate
    ...    {"query_type": "aggregation", "nodes": [{"id": "p", "entity": "Project", "columns": ["name"], "node_ids": [int($project_id)]}, {"id": "v", "entity": "Vulnerability", "filters": $vulnerability_filters or {}}], "relationships": [{"type": "IN_PROJECT", "from": "v", "to": "p"}], "aggregations": [{"function": "count", "target": "v", "alias": "vuln_count"}], "group_by": ["p"], "limit": 10}
    ${resp}=    Orbit Query With Token    ${query}    ${ROLE_AUTHZ_VICTIM_PAT}
    RETURN    ${resp}
