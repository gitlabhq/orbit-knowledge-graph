...
that every binary can serve every archive. Before a cross-version rollout, deploy a preparatory
release that publishes the current archive and supports archive-backed serving, and validate the
intended binary/archive combinations. Publication, rollback, and retention use the existing
