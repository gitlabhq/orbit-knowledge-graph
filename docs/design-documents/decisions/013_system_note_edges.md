...
> **Update (2026-06-22): `ADDS_COMMIT` / `MERGED_AT_COMMIT` now resolve to a real `Commit` node.** The original design parsed commit SHAs but could not emit commit edges because `gl_edge.target_id` is `Int64` and a SHA is a hex string, with no Siphon table to drive a `Commit` node — so commit refs were dropped (`return None` in `resolve.rs`) and the `commit`/`merge` actions folded into the `MENTIONS` catch-all (`gitlab-org/orbit/knowledge-graph#815`). A lightweight **`Commit` node** (`config/ontology/nodes/source_code/commit.yaml`, `source_code` domain) now closes the gap, reusing the `Branch` node's synthetic-id strategy: `id = FxHasher(project_id, sha)` (`compute_commit_id` in `emit.rs`, mirroring `compute_branch_id` in `arrow_converter.rs`). The id is deterministic from `(project_id, sha)`, so commit edges need **no datalake lookup** — `resolve.rs` is unchanged. The `SystemNotesTransform` **dual-writes** `gl_commit` node rows alongside `gl_edge` rows in one pass (Option A), exactly as the code indexer emits `Branch` rows. `Action::Commit` and `Action::Merge` are split out of the `MENTIONS` arm: `commit` → `MergeRequest --ADDS_COMMIT--> Commit` per listed SHA; the SHA-bearing `merge` (auto-merge) variant → `MERGED_AT_COMMIT`; the `"created merge request !123"` `merge` variant stays `MENTIONS`. Short SHAs are stored as-referenced (no Gitaly normalization in v1); a later enrichment pass can upsert full SHAs via `ReplacingMergeTree`.
...
- **Commit references** (`deadbeef`): resolved to a synthetic `Commit` node id (`FxHasher(project_id, sha)`) computed inline — they bypass the `siphon_routes` IN-list scan entirely, so no route lookup is needed (see the 2026-06-22 `ADDS_COMMIT` / `MERGED_AT_COMMIT` update above).
