...
> **Update (2026-06-25): MENTIONS edge direction corrected.** The original implementation set `source = noteable, target = body-ref`, which inverted the edge for all routed cross-reference actions. Rails writes the system note on the *mentioned* entity, with the body naming the *mentioner*, so `row.noteable_id` is the mentioned side and `resolved.id` is the mentioner. The scope-and-edge-kinds table above was always correct (`From → To` = mentioner → mentioned), but the emitter had source/target swapped. The fix swaps `source_id`/`target_id` and `source_kind`/`target_kind`, and changes the edge's `traversal_path` from `resolved.traversal_path` (the mentioner's namespace) to `row.traversal_path` (the noteable's/mentioned entity's namespace), so inbound-degree queries on the target still hit the right `gl_edge` partition. A re-index is required; the data is derived (ReplacingMergeTree overwrite, no migration). See [`dgruzd/tasks#2991`](https://gitlab.com/dgruzd/tasks/-/work_items/2991) for the full investigation.
