...
3. One shape for all query types. Search, traversal, aggregation, path_finding, neighbors all produce `{ query_type, nodes, edges }`. Aggregation queries additionally include `columns` to describe the computed values.
