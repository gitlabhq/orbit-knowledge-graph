...
- **Coverage**: every tag a job pushes is signed by digest: the multi-arch tag, whose `latest` and `dev` aliases share its digest, and the per-arch `-amd64` and `-arm64` tags, which buildx pushes as their own single-platform indexes. Platform manifests inside an index are reached only through a verified index and carry no separate signature. Build-cache images and the e2e robot image are not release artifacts and are not signed.
