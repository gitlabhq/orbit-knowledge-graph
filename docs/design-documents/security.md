...
- **Property grouping keeps protected aliases in SQL**: a property `group_by` entry (`"<node-id>.<property>"`) groups by a property on a node alias, and the lowerer keeps that alias table-backed so the same role-scoped `SecurityPass` predicate applies before aggregation. A Reporter-only user cannot get `Vulnerability.severity` or `SecurityScan.scan_type` buckets from paths that require Security Manager access.
