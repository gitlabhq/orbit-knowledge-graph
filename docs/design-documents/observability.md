...
| `gkg.billing.events.delivery_failed` | Counter | count | `reason` (non_retriable_status / retries_exhausted / auth) | Billing events permanently dropped by labkit's background loop without reaching the collector, via the `on_failure` callback. |
