...
Latency histograms share a small set of named bucket sets (`LATENCY`, `LATENCY_FAST`, `BATCH_SIZE`, `BLOB_BYTES`, `ROW_COUNT`, `MEMORY_BYTES`) defined in [`crates/gkg-observability/src/buckets.rs`](../../crates/gkg-observability/src/buckets.rs); the default `LATENCY` set covers 5 ms to 10 s.
