...
The [Orbit query frontend](orbit_query_frontend.md) is a compiler-level API for Orbit's read-only graph language. Pest pairs become a typed syntax tree that lowers into compiler Input. Remote requests select it with `language: gql`; the JSON Query DSL remains the default.
...
`ListNamedQueries`, surfaced as `GET /api/v4/orbit/templates`, returns only parameterless definitions rendered in the request header's language. Each entry contains only its name, description, and caller-rendered `raw_query`. The default query leads the catalog; the other entries retain name order. Rails supplies the editor mode in the page's bootstrap data, independently of catalog contents. Empty or failed catalogs cannot change the editor mode. Missing request metadata selects JSON for older Rails callers.
