...
The Orbit query frontend accepts a read-only language based on openCypher 9.
It has Orbit-specific restrictions and extensions. It does not claim full language conformance.

The `opencypher` crate provides a compiler-level API. The JSON Query DSL remains the default for remote requests.
...
`crates/query-engine/opencypher/src/query.pest` translates selected productions from the official M23 EBNF grammar into Pest.
...
The crate's `NOTICE` records the source URL, SHA-256, modifications, and upstream attribution.
Its `LICENSE` contains the Apache-2.0 license. These files retain the upstream names unchanged.
...
`opencypher::parse` takes query text and parameters and returns Input.
`opencypher::compile` passes that Input to `compiler::compile_from_input`.
