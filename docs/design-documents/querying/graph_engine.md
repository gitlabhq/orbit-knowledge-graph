...
The in-repo encoder uses `serde-content` to capture Serde values in encounter order. A private Pest grammar (`crates/utils/src/toon.pest`) classifies bare keys and string values and decomposes number tokens for decimal expansion. The Rust writer walks the normalized values, escapes quoted text, and emits objects, lists, and tables. Its fixed profile uses a comma delimiter and two-space indentation. Pest does not serialize the payload or decode TOON documents. Grammar tests cover lexical boundaries; the official encode fixtures and public encoder tests cover output bytes and structure. These tests and the query formatter tests run in the unit-test CI job.

Before encoding, `ToonFormatter` sorts node-property and aggregation-row map keys recursively in its own response. This makes TOON property order independent of `serde_json` features and hash-map iteration order. It preserves array order and struct field order. The shared graph formatter, RAW output, and health metrics are unchanged. The general encoder preserves the key order it receives, as the TOON specification requires.
