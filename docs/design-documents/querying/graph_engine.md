...
| 10 | `rebind` | For a materialized join table plan, rewrites `<node>.<column>` references onto the single scan's `src_`/`tgt_` columns (see [Materialized join tables](#materialized-join-tables)) |
| 11 | `check` | Verifies every node-table alias carries a valid `startsWith` predicate traceable to the `SecurityContext` ([Security](../security.md)) |
...
| Materialized join table scans | Single scan with `FINAL`, `_deleted = false` | The row's `_deleted` already folds in the edge and both endpoints, so one latest-row read replaces the edge scan plus two node joins |
...
### Materialized join tables

An edge variant can opt in with `materialized: true` in its YAML. The ontology then declares a pre-joined `gl_mat_{rel}__{src}__{tgt}` table (`ontology::materialized`) carrying the edge's `traversal_path` and `relationship_kind` plus every storage column of both endpoint nodes under `src_`/`tgt_` prefixes, sorted on `(traversal_path, {anchor}_id, {other}_id)` where the anchor is the scoped side when only one side is scoped. The DDL generator emits the table and three `TO` materialized views, one per source table (edge, source node, target node); whichever side receives an insert re-emits the affected join rows, reading the other two with `FINAL`, and the `ReplacingMergeTree` keeps the row with the greatest `_version`. Because the table inherits the edge's `traversal_path`, at least one endpoint must be scoped; the loader rejects a variant whose endpoints are both global.

The compiler uses the table for a single fixed-length hop that names exactly one relationship kind whose resolved variant is materialized (`normalize` binds the hop, `plan` selects `Strategy::Materialized`). The lowerer emits one `FINAL` scan under the usual `e0` edge alias and synthesizes the per-hop edge columns the formatter expects; node hydration is skipped for both endpoints. Node columns are still emitted against the node aliases through lowering, enforcement, and cursor seek, and the late `rebind` pass maps them onto the scan (`mr.title` becomes `e0.tgt_title`; the join table's own columns such as `traversal_path` rebind without a prefix). The security pass treats the scan like any `gl_` table: `min_access_level_for_table` returns the higher of the two endpoints' floors, and the scan is path-scopable. Variants whose endpoint entity needs an elevated role floor fall back to the edge-chain plan, since that floor is enforced through a node-table subquery the single-scan shape does not have. `REVIEWER` (User to MergeRequest) is the first opt-in; on the benchmark corpus its aggregations run 20 to 40 times faster than the edge join.
