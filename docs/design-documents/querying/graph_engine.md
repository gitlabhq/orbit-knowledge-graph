...
Orbit Remote caps complete encoded query results at `grpc.max_query_response_bytes` (8 MiB by default). Oversized cursor pages return a fitting prefix without splitting cursor-key groups, with `next_cursor` rebuilt from retained rows. Oversized indivisible or non-cursor results return `result_too_large`. Sizing reuses hydrated rows within the query timeout, without additional SQL.
