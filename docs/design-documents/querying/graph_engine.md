...
| 10 | `rebind` | For a denormalized join table plan, rewrites `<node>.<column>` references onto the single scan's `src_`/`tgt_` columns (see [Denormalized join tables](#denormalized-join-tables)) |
