...
Orbit Remote also bounds each query result by `grpc.max_query_response_bytes` (8 MiB by default). The budget covers the complete encoded `ExecuteQueryMessage`, including RAW or GOON content, pagination, format metadata, and debug SQL. After hydration, response construction reuses borrowed row prefixes without issuing additional queries. Sizing first estimates a prefix from the full response size and probes its adjacent boundary. If those probes do not locate the boundary, binary search finishes the search over payload sizes without pagination. Candidates are then checked from largest to smallest with their actual cursors, whose lengths need not increase with the row count. RAW responses serialize the graph directly to JSON text without building an intermediate JSON value tree.
