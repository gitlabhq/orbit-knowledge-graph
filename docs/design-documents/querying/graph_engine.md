...
| Denormalized join table scans | Single scan with `FINAL`, `_deleted = false` | The row's `_deleted` already folds in the edge and both endpoints, so one latest-row read replaces the edge scan plus two node joins |
...
An edge variant can opt in with `denormalized: true` in its YAML. The ontology then names a pre-joined `gl_denorm_{rel}__{src}__{tgt}` table (`ontology::denormalized`) and the DDL generator composes it from the already-generated definitions of the edge table and the two node tables: every edge column as is, then every node column under a `src_`/`tgt_` prefix, then the system columns once. Codecs, types, and defaults are therefore identical to the source tables by construction. A node's `id` is not copied, since it is already the edge's `source_id`/`target_id`. The sort key is `(traversal_path, {anchor id}, {other id})` where the anchor is the scoped side when only one side is scoped. The DDL generator emits the table and three `TO` materialized views, one per source table (edge, source node, target node); whichever side receives an insert re-emits the affected join rows, reading the other two with `FINAL`, and the `ReplacingMergeTree` keeps the row with the greatest `_version`. Because the table inherits the edge's `traversal_path`, at least one endpoint must be scoped; the loader rejects a variant whose endpoints are both global.

Because the table is a superset of the edge table, the compiler does not need a separate lowering for it. `normalize` binds a hop that names exactly one relationship kind to the variant's table (respecting direction, so either query node may play the variant's source), and `plan` points the hop's `edge_table` at it when the shape is safe: a fixed-length hop whose endpoints use the default redaction id and no elevated role floor (either would force a node-table read anyway). The hop may sit anywhere in a chain; the other hops are unaffected. When every other hop in the chain is FK-backed, the FK chain lowering takes over instead: the planner rotates the denormalized hop to the front (moving its relationship with it so the `e{i}` output columns stay paired), roots the chain on the join table scan, which reaches both of its endpoints at once, and joins the remaining node tables on their FK columns, so `u -REVIEWER-> mr -TRIGGERED-> pl` becomes the join table joined to `gl_pipeline` on `pl.merge_request_id = e0.target_id` with no edge scan and no `gl_user`/`gl_merge_request` scan. The ordinary flat edge-chain lowering then runs with three differences: the hop scans with `FINAL` instead of the LIMIT BY dedup, because a row changes whenever either node does (multi-hop chains already dedup every hop with `FINAL`); node filters are emitted as predicates on the hop's typed node columns instead of edge tag lookups, both on the main scan and inside cascade anchors; and both endpoints skip hydration, with their requested columns selected from the hop alias. Lowering finishes by rebinding every `<node>.<column>` onto the hop alias (`mr.title` becomes `e0.tgt_title`, `u.id` becomes `e0.source_id`). No later pass introduces node-alias columns: `enforce` reads the plan's node-to-edge mappings, `cursor` derives its seek from the lowered ORDER BY, and `security`, `partition`, and `check` work on scan aliases. The security pass treats the scan like any `gl_` table: `min_access_level_for_table` returns the higher of the two endpoints' floors, and the scan is path-scopable. `REVIEWER` (User to MergeRequest) is the first opt-in; on the benchmark corpus its aggregations run 20 to 40 times faster than the edge join.
