...
The compiler uses the table for a single fixed-length hop that names exactly one relationship kind whose resolved variant is denormalized (`normalize` binds the hop, `plan` selects `Strategy::Denormalized`). The lowerer emits one `FINAL` scan under the usual `e0` edge alias and synthesizes the per-hop edge columns the formatter expects; node hydration is skipped for both endpoints. Node columns are emitted against the node aliases as usual, and lowering finishes by rebinding them onto the scan (`mr.title` becomes `e0.tgt_title`; the join table's own columns such as `traversal_path` rebind without a prefix). No later pass introduces node-alias columns: `enforce` reads the plan's node-to-edge mappings, `cursor` derives its seek from the lowered ORDER BY, and `security`, `partition`, and `check` work on scan aliases. The security pass treats the scan like any `gl_` table: `min_access_level_for_table` returns the higher of the two endpoints' floors, and the scan is path-scopable. Endpoints that would need a node-table read anyway fall back to the edge-chain plan: an elevated role floor is enforced through a node subquery, and a non-default redaction id column makes `enforce` join the node table. `REVIEWER` (User to MergeRequest) is the first opt-in; on the benchmark corpus its aggregations run 20 to 40 times faster than the edge join.
