...
**Payload propagation.** The same tight prefix extends from the scoped anchor to the payload entities a query actually returns, so the large tables (work items, merge requests, vulnerabilities, definitions, files) prune on their own primary key rather than only the anchor's small dimension table. Eligibility is derived from the ontology, never enumerated: an alias may receive the prefix when its entity is path-scopable (`Ontology::is_path_scopable` — namespaced with a `traversal_path`-leading primary key; `gl_user`/`gl_runner` are global and never qualify) and it is reachable from a scope anchor (`Ontology::is_anchor`, reusing the existing `traversal_path_lookups` config) over a path of scope-preserving edges only. The scope-preserving edge set is itself derived from the ontology's containment model (`Ontology::scope_preserving_edge_triples`) — the namespace-of-record foreign key (`IN_PROJECT`, `IN_GROUP`) and structural downward containment (`Group → Project`, `Project → Branch → Directory → File`, intra-project `DEFINES`/`CALLS`/`EXTENDS`) — not a hand-maintained edge list. A single transitive-taint reachability walk over the query's join graph enforces the boundary: the moment a path crosses a cross-namespace-capable edge (`RELATED_TO`, `CLOSES`, `MENTIONS`/notes, a fork's `SOURCE_PROJECT`, an epic's `CONTAINS` to a child in another project, a multi-project pipeline trigger) that alias — and everything reachable only through it — keeps the broad authorization filter alone, so cross-namespace joins and relationships return exactly as before. An anchor node never inherits a neighbor's prefix, so a sibling project under a shared group is never over-pruned. Edge-table scans never receive a tight prefix. The propagation sits behind the default-off `traversal_path_payload_scoping` feature flag, separately killable from anchor scoping.
