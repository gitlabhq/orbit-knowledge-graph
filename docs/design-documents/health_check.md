...
The watcher follows the active ontology archive rather than comparing its version with the
binary. Metadata-read errors retain the previous snapshot. No active version, or an archive load,
parse, or ontology failure for the confirmed active version, clears it. Archive failures are
retried without restart; a newer active version does not trigger a shutdown. See
[schema management](schema_management.md#webserver-readiness-gate) for the installation and recheck
rules.

While pending, schema-dependent RPCs return `Unavailable`. Static tool and command listings,
response-format and Query DSL metadata, and cluster health remain independent of the snapshot.
Requests already holding a snapshot continue with it, subject to their deadline and table retention.
