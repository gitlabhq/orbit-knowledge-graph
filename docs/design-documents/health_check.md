...
| `tls.http.enabled` | Serves the HealthCheck listener over TLS. The URL above must then be `https://`, and the certificate needs a SAN for the HealthCheck service name because the Webserver verifies it against the OS trust store. |
