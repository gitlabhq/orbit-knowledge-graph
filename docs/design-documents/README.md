...
- **Orbit Remote's service binary**, with four runtime modes:
  - **`Webserver`** (`gkg-server --mode Webserver`): Serves HTTP, gRPC, REST, and MCP traffic; validates query requests against their frontend and the ontology; compiles them to ClickHouse SQL; and applies authorization and formatting before returning results.
