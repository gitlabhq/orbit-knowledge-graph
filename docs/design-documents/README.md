...
- Archive-backed remote serving: each schema-dependent request pins the active archive's ontology, table prefix, compatible named queries, and path resolver. The Webserver builds and atomically swaps snapshots on promotion or rollback without restarting for compatible archives. See [schema management](schema_management.md#webserver-serving-snapshots) for readiness, rollout prerequisites, and in-flight table-retention limits.
