...
The shared `orbit context` command routes bare Definition references and literal file paths to Orbit Local. Other Ontology node database-ID references, including typed File references, route to `/api/v4/orbit/context` in one authenticated GET with repeated `refs[]` parameters. The CLI normalizes the `Issue` shorthand to `WorkItem`, then validates node names with the existing read-only `Ontology::get_node` lookup. This preserves the database ID without another API call. The Ontology and graph-query validation are unchanged; no Issue node is added. Both `Type:ID` and quoted `Type[ID]` normalize to canonical `Type[ID]` on the wire.

The CLI classifies the entire batch before telemetry, credentials, HTTP, or DuckDB access, reusing that classification for dispatch and telemetry. Mixed batches are supported. The local subset remains definitions OR one file; `--repo` and `--db` apply only there, and `--tests` is definition-only. Remote-only calls reject unused local options and need no checkout or database. Bare typed refs take priority over same-named files regardless of `--repo`; explicit paths select local context and resolve from the Git root.

Local text is emitted first, followed by a separator and the unchanged remote response bytes (`llm` by default, or `json`). Mixed stdout is composite text even with remote JSON; remote-only JSON remains a whole machine-readable payload. Local-only output is unchanged and rejects `--response-format`. Either resolver failing exits nonzero without fallback: local failure prevents the remote call, while remote failure can leave partial local stdout and the separator. Per-entity misses in a successful remote response remain successful.

The adapter does not interpret GOON, redact fields, follow links, or add a context route to the service. Node support and authorization require a supporting GitLab API deployment. Bare Definition misses stay local; scoped remote Definition resolution is a separate API contract dependency, not implemented here.
