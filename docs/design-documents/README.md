...
- **Orbit Remote's service binary**, with four runtime modes:
  - **`Webserver`** (`gkg-server --mode Webserver`): Serves HTTP, gRPC, REST, and MCP traffic. It validates JSON Query DSL or explicitly selected GQL requests against their frontend rules and the ontology. It compiles them to ClickHouse SQL. It applies authorization and formatting before returning results.
