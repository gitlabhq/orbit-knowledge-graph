...
- **Knowledge Graph service** as a unified binary with four runtime modes:
  - **`Webserver`** (`gkg-server --mode Webserver`): Serves HTTP, gRPC, and MCP traffic; validates graph queries against the JSON schema and ontology; compiles them to ClickHouse SQL; and applies authorization and formatting before returning results. Also exposes `GetGraphStatus`, which reads indexing progress from NATS KV (see [ADR 009](decisions/009_indexing_progress_nats_kv.md)).
