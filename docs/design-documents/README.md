...
- Remote HTTP, gRPC, REST, and MCP query surfaces that compile the JSON Query DSL (default) or GQL text, selected per user by the Rails rollout flag, through the [Orbit query frontend](querying/orbit_query_frontend.md) into parameterized ClickHouse SQL.
