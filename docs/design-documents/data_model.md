...
The data model is designed to be intuitive and to mirror the mental model that developers and users have of the GitLab platform. By representing entities as nodes and their interactions as relationships, we can perform complex queries. These queries would be difficult or inefficient with a traditional relational database.
...
- **Unified ontology and shared graph primitives**: Both the Code Graph and the SDLC Graph use the same ontology-driven entity and relationship model. It is defined in `config/ontology/`, with the same ClickHouse graph schema in `config/graph.sql`. Edges are stored in ontology-configured edge tables (defaulting to `gl_edge`). Each edge YAML can set a `table:` field to route specific relationship types to dedicated tables. This allows linking between the two graphs. For example, a `Project` node from the SDLC graph can be linked to a `Branch`, `File`, or `Definition` from the Code Graph.
- **Entity as Node**: Every entity in the GitLab platform (e.g., Project, Issue, File, Function Definition) is represented as a node.
...
The primary goal is **ingest/merge isolation**. Without partitioning, all ~1,100 top-level namespaces share one part budget per table. So one tenant's reindex burst can exhaust `parts_to_throw_insert` and dead-letter inserts for every tenant. Bucketing gives each bucket its own part budget and merge scope, so a busy tenant throttles only its own bucket. The partition is a deterministic function of `traversal_path`, and every sort key leads with `traversal_path`. So all versions of a row land in the same bucket. This keeps `ReplacingMergeTree FINAL` dedup correct.

The hash scatters prefixes across buckets, so a `startsWith(traversal_path, …)` filter alone cannot prune partitions. Wherever a scan's filter confines an alias to one or more top-level namespaces via `startsWith(alias.traversal_path, '<prefix>')`, the compiler ANDs a predicate on ClickHouse's `_partition_id` virtual column. This column holds the per-part formatted partition value, resolved from part metadata with no column read. The bucket constant is computed in ClickHouse from the prefix via the same strategy expression the DDL uses, so the two are byte-identical by construction:
