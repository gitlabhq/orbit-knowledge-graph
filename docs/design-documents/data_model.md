...
| `MERGED_AT_COMMIT`                  | `MergeRequest` | `Commit`       | The commit a merge request was merged at, sourced from the scalar FK `COALESCE(merged_commit_sha, merge_commit_sha, squash_commit_sha)` on the `merge_requests` row (never the auto-merge system note), gated on merged state. `merged_commit_sha` is hex-of-ASCII `bytea` (unhex-decoded); `squash_commit_sha` is raw-binary `bytea` (`\x` stripped); both decode to clean SHAs. Write-once FK (immutable). About 9.4% of merged MRs (fast-forward merges, very old rows) carry no SHA in any column and get no edge. |
...
| `Commit`              | A Git commit, shared target for commit-related edges (MERGED_AT_COMMIT, CONTAINS_COMMIT per #925). Currently emitted from the `merge_requests` row; synthetic `id` = `sipHash64(target_project_id, sha)` (no Siphon source table of its own). | `id`, `sha`, `project_id`                                  |
