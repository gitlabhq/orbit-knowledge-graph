...
Every graph table that carries `traversal_path` (all node tables except the global hubs `User` and `Runner`, plus every edge table) is `PARTITION BY` a hash bucket of the top-level namespace, unless it is listed in `settings.partition.exclude`. Tables too small to hit part-count pressure (e.g. `gl_project`, `gl_group`, `gl_milestone`) are excluded: partitioning them only adds per-partition metadata overhead, so they stay single-partition `ReplacingMergeTree`s. The strategy is declared once in `config/ontology/schema.yaml` under `settings.partition.strategy` and rendered into `config/graph.sql`:
