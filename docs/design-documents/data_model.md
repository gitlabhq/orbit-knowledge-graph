...
| `MERGED_AT_COMMIT`                  | `MergeRequest` | `Commit`       | The commit a merge request was merged at, sourced from the scalar FK `COALESCE(merged_commit_sha, merge_commit_sha, squash_commit_sha)` on the `merge_requests` row (never the auto-merge system note), gated on merged state. `merged_commit_sha`/`squash_commit_sha` are `bytea` (Siphon stores them `\x`-hex-encoded) and are decoded to clean SHAs. Write-once FK (immutable). About 9.4% of merged MRs (fast-forward merges, very old rows) carry no SHA in any column and get no edge. |
