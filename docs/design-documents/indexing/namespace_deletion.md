...
The `Project` and `Group` extracts collapse their route-table join with `argMax(traversal_path, version)` scoped to the root being indexed, so a move *within* a root no longer emits the entity under both paths. Two cases still reach the reconciler: rows written before that collapse existed, and moves *across* roots, where the stale path is the only one the root-scoped extract can see and the entity keeps being re-emitted there. Collapsing globally instead would cost a per-page `id IN (…)` lookup against the whole route table — measured at roughly 400× the rows read — so the reconciler remains the removal path.
