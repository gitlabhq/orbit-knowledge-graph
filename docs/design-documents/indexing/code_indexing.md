...
- **Markdown** is parsed with comrak (CommonMark) through the generic pipeline's `LanguageParser::Custom` hook rather than a tree-sitter grammar. Every heading becomes a `Section` definition; nesting follows heading levels, and sibling headings with the same text get a numeric FQN suffix. YAML front matter is consumed so it does not surface as a heading. Machine-translated documentation trees (`doc-locale/`, `docs-locale/`) are excluded from parsing and remain bare `File` nodes.
