...
- **TypeScript and JavaScript** use OXC for parsing and semantic analysis, with Vue/Svelte files contributing their extracted script blocks to the same JS pipeline. Template-only Vue/Svelte files are still indexed as file nodes even when no script block exists. The JS linker distinguishes true invocation sites from plain identifier reads, keeps imported-symbol provenance attached to the correct imported binding even when multiple names share one statement span, records namespace imports as file-backed provenance edges, falls back to file-backed import provenance when a module path resolves but a specific export binding cannot be recovered, resolves nested namespace/member chains through re-export barrels, and skips minified files. Vue Options API extraction runs for real `.vue` SFCs and explicit wrapper APIs such as `defineComponent(...)` or `Vue.extend(...)`; actual SFC `export default { ... }` shells produce a component definition even when they only declare props/emits/name, while non-`.vue` files still require a recognized wrapper or executable option surface to avoid misclassifying plain object exports.
- **GraphQL, JSON, and SVG** files are indexed as file-backed documents so JavaScript imports like `import query from "./query.graphql"` or `import schema from "./schema.json"` resolve to the underlying file node even though the current graph does not extract internal definitions from those asset/document formats yet.
- **Python, Kotlin, Java, C#, and Rust** use tree-sitter grammars.

Language detection is extension-based. Ruby, TypeScript/JavaScript, Python, Kotlin, and Java support full reference extraction. C# and Rust currently support definitions and imports only. GraphQL, JSON, and SVG currently participate as file-backed import targets.
