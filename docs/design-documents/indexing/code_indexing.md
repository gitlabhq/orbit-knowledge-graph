...
Dropping the run future does not stop the parse: it runs on a blocking thread with its own worker pool, so the handler also raises a cancellation token the language pipelines poll at their per-file loop boundaries. The JavaScript and Rust pipelines return without emitting a graph; the generic pipeline stops per-file work and emits whatever partial graph it holds, which the handler then refuses to checkpoint. Either way the worker pool is released rather than left pinned to a job nothing is waiting for. Resolution of a file already in flight is bounded by the per-file watchdog (`per_file_timeout_ms`), which arms a kill flag the resolvers poll. A parse already in flight is not interruptible, so one file's parse is the floor on how fast cancellation can take effect.
