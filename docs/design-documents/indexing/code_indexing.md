...
Tombstones only hide the previous snapshot. Every row of one run carries `_version = indexed_at`, so the table cleanup task (see [SDLC indexing](sdlc_indexing.md), "Table cleanup") removes the rows physically: for each project checkpointed since the pass before its previous one it issues one patch-part delete per code table for all rows with `_version` below the checkpoint's `indexed_at`, which covers the previous snapshot, any orphan rows of runs that never checkpointed and the tombstones themselves. Reads are unaffected because a patch-part delete masks rows immediately and creates no mutation.
