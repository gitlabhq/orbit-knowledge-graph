...
Alongside the per-file CPU budget (which aborts a single slow file's parse), a per-repository source-byte cap bounds peak memory. When the inventory is grouped into parse candidates, the pipeline accumulates their source bytes in path-sorted order; once the accepted bytes reach `max_parse_bytes`, every remaining candidate is shed before it is parsed. Because the shed happens at inventory time rather than mid-parse, it covers every language uniformly (including the JavaScript, TypeScript, and Rust families, whose custom pipelines never observed the earlier in-parse budget) and it is deterministic — the canonical inventory is path-sorted, so the same commit sheds the same files every run. Shed files still appear as unparsed `File` nodes, exactly like the `max_files` cap. The shed count is reported once per repository as a `memory_budget` skip (`code_files_skipped_total{reason="memory_budget"}`) with one WARN. The cap defaults to 64 MiB of parse-candidate source (`default_code_indexing_max_parse_bytes`); `0` disables shedding. The default's sizing — half a 1.5 GiB per-worker share divided by the worst measured ~12x RSS-to-source ratio — is recorded on that function.
