...
The edge tables (`gl_code_edge`, `gl_edge`) are `ReplacingMergeTree`s partitioned by a hash bucket of the top-level namespace (see [Namespace partitioning](../data_model.md#namespace-partitioning)), so each bucket has its own part budget. Within a bucket, every project's inserts still share that budget. During a full reindex the long tail of tiny repositories (most projects emit fewer than 500 edges) would otherwise produce one small part per table per project, outrunning ClickHouse merges until part-count backpressure throttles inserts.

To avoid this, all code jobs stream their edges into one process-wide `BufferedWriter` (a coalescing layer over `ClickHouseWriter`). A table flushes when it reaches `write_slice_rows` (default 1M), on the soft `write_buffer_age_secs` tick (default 60) but only once the table holds at least `write_min_flush_rows` (default 10k), or when its oldest unflushed row exceeds the hard `write_max_flush_age_secs` cap (default 120). The size-blind age tick was the original leak: in a quiet window it flushed every table at a few hundred rows, so the long tail still produced tiny parts. The floor holds a sub-threshold table across soft ticks so small repositories pool into one well-sized part; the hard cap bounds how long any row waits (and thus the uncheckpointed-rows window on a pod crash) regardless of size. Big repositories cross the row cap on their own and flush promptly. Each submitted batch carries a `FlushToken` the writer invokes once that batch's part lands (or fails), so a producer learns durability per part without the writer knowing what the rows mean. Up to `write_max_concurrent` (default 4) parts are written to ClickHouse at once; they may land out of order, which is safe because the edge tables are `ReplacingMergeTree`s keyed by row identity + `_version` and stale cleanup keys on `indexed_at`, so nothing depends on insert order. The bound trades memory (that many `write_slice_rows`-sized parts in flight) for write throughput.

Each part insert is retried with bounded backoff (2s then 4s) before the part is declared failed, so a transient ClickHouse blip does not cost the project a full re-index on the next sweep tick. Only after the retries are exhausted is the part's token marked failed. The retry runs through the shared engine retry harness (`engine/retry.rs`): a local `RetryPolicy` driven by `drive`, the same vocabulary the SDLC datalake read, the schema-readiness wait, and the job-timeout policy use.

Concurrency is split into two lanes by parsable-file count (`Decision::Parse` entries, not raw file count, so a repository of images or lockfiles is not misclassified as large): a wide lane (`small_indexing_slots`, default 6) for the small-repo tail and a reserved lane (`big_indexing_slots`, default 2) so a flood of small repositories cannot starve monorepos.
...
Stale cleanup is split into a logical and a physical step. The logical step runs in the finalizer: one `SELECT ... FINAL` per code table returns only the keys of the prior snapshot that the new snapshot did not re-emit (surviving keys already outrank their old rows by `_version`), and those keys are inserted back as tombstones. The finalizer then appends one row to the unversioned
`code_stale_snapshots` ledger with the prior `indexed_at`. The ledger row is written before the tombstone scans so a failed scan cannot lose the retirement. The physical step is the dispatcher's
`maintenance.stale_reclaim` task (hourly by default, shared with the SDLC stale row ledger described in the SDLC design document): per graph table it skips the pass when `system.mutations` still holds an unfinished mutation for that table, otherwise it reads the ledger rows retired since the table's high-water mark (up to `max_keys_per_statement`, default 5000, older than `settle_secs`), looks up which `_version` values at or below each retired `indexed_at` still exist for that project, split into live rows and tombstones, and issues a single lightweight `DELETE` whose predicate is a literal list of
(`traversal_path`, `project_id`, `branch`, `_version`) tuples, with `lightweight_deletes_sync = 0`. The pass removes the live rows; the tombstones that hid them are removed one pass later, once the previous mutation has finished, so a read can never see a hidden row whose tombstone is already gone. Every row of a snapshot shares its `indexed_at` as `_version`, so the version list names the retired snapshots exactly, including rows whose keys changed between snapshots and which ReplacingMergeTree can never collapse, and it also picks up older snapshots that were never recorded, such as history from before this task existed. The shared `gl_edge` table has no project or branch columns, so its predicate is
(`traversal_path`, `_version`) plus the code source kinds. One statement per table per pass, covering both ledgers and both phases, keeps the mutation count independent of the reindex rate, the literal list needs no subquery replay per part, and the backpressure check (per replica, as `system.mutations` reflects the replica that answered) keeps this task from stacking mutations. In the default `alter_update` mode ClickHouse re-reads the predicate columns of every part that holds a matched row, so the cadence trades reclaim latency against IO on large tables. Marks are keyed by ledger, phase and logical table name so a schema bump
continues from the previous high-water mark; ledger rows expire after 30 days.
