...
Automatic v2 language dispatch is extension-based. The JavaScript pipeline owns `.js`, `.jsx`, `.mjs`, `.cjs`, `.vue`, `.graphql`, `.gql`, and `.json`; the TypeScript pipeline owns `.ts`, `.tsx`, `.mts`, and `.cts`; the Rust pipeline owns `.rs`; Swift owns `.swift`; Lua owns `.lua`. Ruby, JavaScript/TypeScript, Python, Kotlin, Java, PHP, Elixir, Swift, and Lua support full reference extraction (PHP resolves `$this`/`self`/`parent`/`static` member and static calls, trait/interface inheritance via `Extends`, and `use` imports; Swift resolves `self`/`super` member calls; Lua resolves dot and colon method calls with receiver tracking). Rust emits `CALLS` edges from rust-analyzer semantic resolution and local SSA flow; it does not currently materialize arbitrary non-call reference edges. C# currently supports definitions and imports only. Bash/Shell supports function definitions and `source`/`.` sourced-script imports only; cross-file call resolution, variable bindings, and aliases are out of scope. Swift v1 indexes classes, structs, enums, protocols, functions, constructors, and enum entries; extension-scope reattachment is a planned follow-up. YAML (`.yml`, `.yaml`) indexes anchors as definitions and aliases as references in every file. Mapping keys become definitions and imports only where an embedded document-type config claims them; a document type is one YAML file under `crates/code-graph/src/v2/langs/generic/yaml/document_types/`, validated at load against `config/schemas/yaml_document_type.schema.json`, that declares a matcher (filename suffixes/prefixes or top-level key conditions such as `kind`/`apiVersion`) and the shapes it claims (`root_keys` minus reserved `keywords`, `children_of`, `items_of`, `value_of`, `all_keys`; import keys with scalar and mapping forms). Shipped document types: GitLab CI (`CiJob`, `CiStage`, `CiVariable` definitions; every `include:` flavor plus `trigger.include` as typed imports), ArgoCD (`ArgoCdApplication` spanning the document; `source`/`sources` as `ArgoCdSource` imports), Helm `Chart.yaml` (`HelmChart`; `dependencies` as `HelmChartDependency` imports), Helm `values*.yaml` (`HelmValue` for every key at every depth with dotted FQNs), and Docker Compose (`ComposeService`, `ComposeVolume`, `ComposeNetwork`). Definitions and imports carry real source ranges. Unclaimed YAML keeps its `File` node and anchor/alias def/refs only, as do files above a YAML-specific parser size gate (machine-generated dumps such as openapi exports). Adding a document type is a config file, not Rust. No cross-file resolution is performed.
