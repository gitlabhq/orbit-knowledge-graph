...
To avoid this, all code jobs stream their edges into one process-wide `BufferedWriter` (a coalescing layer over `ClickHouseWriter`) under a per-project sequence number. A table flushes when it reaches `write_slice_rows` (default 500k) or after `write_buffer_age_secs` (default 60), so the long tail coalesces into well-sized parts while big repositories (which cross the row cap on their own) still flush promptly. The writer exposes a `flushed` watermark: the highest sequence number that is finished (all its batches submitted), fully written, and below every still-buffered sequence number and any failed flush.
...
A project is checkpointed only after its rows are durable. The handler parses, submits its batches, registers its checkpoint, finishes its sequence number, and returns without waiting for the write. A background task watches the `flushed` watermark and writes the checkpoint of every project at or below it, in order. If the process dies before that flush, the project is simply never checkpointed and the once-a-minute backfill sweep re-dispatches it (re-indexing is idempotent). A failed flush is logged and its sequence numbers are poisoned, so those projects never checkpoint and are re-indexed. This trades a small window of redone work on a crash for a simple write path: no durability handshake, no lease heartbeat, no lock renewal.
