...
Stale cleanup is split into a logical and a physical step. The logical step runs in the finalizer: one `SELECT ... FINAL` per code table returns only the keys of the prior snapshot that the new snapshot did not re-emit (surviving keys already outrank their old rows by `_version`), and those keys are inserted back as tombstones. The finalizer then appends two rows to the unversioned
`code_stale_snapshots` ledger: the prior `indexed_at` and the version its tombstones were written with (one microsecond below it), so both the retired rows and the tombstones that hid them are reclaimed once the snapshot is retired. The ledger rows are written before the tombstone scans so a failed scan cannot lose the retirement. The physical step is the dispatcher's
`maintenance.code_stale_reclaim` task (hourly by default): per code table it skips the pass when `system.mutations` still holds an unfinished mutation for that table, otherwise it reads the ledger rows retired since the table's high-water mark (up to `max_scopes_per_statement`, default 5000, older than `settle_secs`) and issues a single lightweight `DELETE` whose predicate is a literal list of
(`traversal_path`, `project_id`, `branch`, `_version`) tuples, with `lightweight_deletes_sync = 0`. Every row of a snapshot shares its `indexed_at` as `_version`, so the tuple list names the retired snapshots exactly, including rows whose keys changed between snapshots and which ReplacingMergeTree can never collapse. The shared `gl_edge` table has no project or branch columns, so its predicate is
(`traversal_path`, `_version`) plus the code source kinds. One statement per table per hour keeps the mutation count independent of the reindex rate, the literal list needs no subquery replay per part, and the backpressure check (per replica, as `system.mutations` reflects the replica that answered) keeps this task from stacking mutations. Marks are keyed by the logical table name so a schema bump
