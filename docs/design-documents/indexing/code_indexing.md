...
After acquiring the lock, the service downloads the full repository archive from the Rails internal API. Each run extracts into its own unique directory (keyed by a per-run id, not just project + branch), so two workers racing the same repository — which can happen when the ack deadline lapses mid-unpack and the task is redelivered before the first worker finishes — never share an extraction tree and cannot clobber each other's files. During archive extraction, the Gitaly archive root directory (`<slug>-<ref>/`) is stripped so that indexed paths are repo-relative and match the paths used by content resolution's `list_blobs` revisions. After indexing completes (or fails), that run's extraction directory is immediately deleted from disk to prevent unbounded storage growth across indexer pods.
