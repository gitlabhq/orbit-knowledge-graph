...
Rows deleted in the source database have `_siphon_deleted` set to `true`. The extraction query pulls these rows alongside live data, and the `_deleted` flag carries through to the graph table as a tombstone row. Downstream queries resolve versions and filter on the flag, so a tombstone hides the key at once. Tombstones and the rows they hide are removed physically by the table cleanup task described below.

**Table cleanup**

`TableCleanup` is a `ScheduledTask` in `DispatchIndexing` mode (default every 10 minutes). It removes dead rows without mutations, using ClickHouse patch-part deletes (`lightweight_delete_mode = 'lightweight_update_force'`, ClickHouse 25.7.8, 25.8.8, 25.9.3, 25.10.1 or newer). Since schema 94 the generated DDL declares `enable_block_number_column` and `enable_block_offset_column` on every versioned table, so each inserted row carries a persisted `(_block_number, _block_offset)` identity that merges preserve. A patch-part delete writes one small part that masks the matched rows atomically and immediately, including under `FINAL`, and never rewrites the parts of the table; the mask is folded into the parts by regular merges and by `APPLY PATCHES`, which the task issues when patch parts grow past a size or age threshold, checked after every statement so that a first pass over a multi-billion-row table does not postpone it for hours.

The task only runs on an active schema version. While a migration is still building the tables of the pinned version it does nothing: during a backfill the tombstones of the datalake arrive across every path at once, so no statement could be pruned, and the first pass after activation sweeps the finished tables whole instead.

For every graph table (`concurrent_tables` at a time, default 4) the task collapses keys that received a tombstone since its previous pass. Candidates are the tombstoned keys in parts whose name ends above the block-number cursor of two passes ago (one pass of overlap covers merges that complete between passes), with `_block_number` above that cursor. One statement per path group deletes every row of those keys except the newest row: one aggregation of the pruned range decides what each candidate key keeps, and the rows to delete come from a pre-filtered view of the same range joined to it, so a statement reads its range three times in total. The newest tombstone is kept: it keeps shadowing late rows that arrive with an older `_version`, such as a lagging SDLC page for a namespace that was deleted or moved. Tombstones older than `tombstone_retention_secs` (default 7 days) are purged once per `purge_interval_secs` (default daily) together with everything they hide, grouped by traversal path so every statement is pruned by the primary key. Statements run with `update_sequential_consistency = 0`, because the default makes a lightweight update wait behind every pending replication-queue entry and the block-number cursor overlap already covers parts a replica fetches late. Every statement also names the traversal paths of its candidates as literals (up to an eighth of the session's `max_query_size`, between 32 KiB and 1 MiB), because a lightweight delete does not use an `IN (subquery)` set for primary-key pruning; a table that needs more than eight statements in one pass has tombstones on most of its paths, and from there each statement is pruned by a contiguous path range instead, so the sweep reads the table a few times in total rather than once per statement; without the list a statement scans the whole table (measured on a 1B-row table: 54 s and 1.15 GiB per statement, against 1.9 s and 40 MiB with the list). A live row tied with a tombstone at the same `_version` counts as live and stays. Cursors (highest block number seen, pass time, last purge, hold) live in the `checkpoint` table under `maintenance.table_cleanup.<versioned table>`, so a schema version starts from scratch: the first pass for a table purges expired tombstones and collapses every remaining tombstoned key (`sweep_history`), then continues incrementally.

A patch written against a part that is being merged or mutated at that moment is applied to the result part in join mode, matching rows by `(_block_number, _block_offset)` with a hash table that every read over the part rebuilds and that `APPLY PATCHES` probes single-threaded; on a multi-billion-row part that takes hours. Before each table's statements the task therefore lists the source parts of the merges in flight on every replica (`clusterAllReplicas(<merges_cluster>, system.merges)`, or the local `system.merges` when no such cluster is defined; on ClickHouse Cloud each replica only sees its own merges) and excludes them from the outer predicate with `_part NOT IN (...)`; a table with a pending mutation, including the task's own `APPLY PATCHES`, is skipped for the pass, because every part still queued for the mutation is renamed when its turn comes. Rows left behind in busy parts are caught up later: a pass that excluded parts records the start of its window as a hold, later passes keep their normal windows while the hold lasts, and the first pass that finds the table quiet re-reads from the hold once and clears it. Only a merge that starts during the seconds a statement runs can still produce a join-mode patch, and it is bounded by the statement's candidate cap.

For code tables the task uses the code checkpoint as the truth instead of the tombstone collapse: for every `(traversal_path, project_id, branch)` whose checkpoint row was inserted since the pass before the previous one (the same overlap as for tombstones) it deletes all rows with `_version` below the checkpoint's `indexed_at`, which removes the previous snapshot, orphan rows of failed runs and the cleaner's tombstones in one statement per table, pruned by the primary key to the affected projects. A scope without a live `Branch` row at or after the checkpoint's `indexed_at` recorded an empty repository, possibly after a transient fetch failure, and is skipped. The first pass sweeps every scope that holds more than one snapshot version in the file table (the smallest table every snapshot writes; a scope missed there is swept at its next re-index), in chunks of 500; scopes with a single snapshot have nothing to remove. Scopes are discovered once per pass and embedded in each statement as literal tuples with their bounds, and the target table is joined through a pre-filtered derived table, because joining it directly made ClickHouse read the whole table once more without a key condition. On the shared `gl_edge` table only paths with a single indexed branch are cleaned, bounded by that branch's checkpoint, and only rows of the code node kinds are touched.

Two ClickHouse constraints shape the migration path. `ATTACH PARTITION FROM` refuses a source table with pending patch parts, so the clone step of a schema migration applies pending patches first. A patch applied to a part that was merged or mutated after the patch was written matches rows by `(_block_number, _block_offset)`, so that identity must be unique within a table. Two states break it: parts attached from another table keep the source's block numbers while the new table numbers its own inserts from zero, and ClickHouse Cloud persists `_block_offset` on merged parts while `_block_number` stays virtual, so rows from different source blocks share an identity. The task checks both the first time it sees a table in a schema version, from `system.parts_columns` and from the part names, re-checks the offset-only state on every pass, and records the verdict in the `checkpoint` table under `maintenance.table_cleanup.identity.<versioned table>`; a refused table stays refused for the whole version, because attached parts stop looking foreign once the table's own numbering passes them. Deleting the identity row re-checks a table after it was rebuilt.

Row-level `TTL` and `OPTIMIZE ... FINAL CLEANUP` are not used: a TTL cannot express "superseded", and a cleanup merge of a multi-billion-row part does not finish.
