...
For code tables the task uses the code checkpoint as the truth instead of the tombstone collapse: for every `(traversal_path, project_id, branch)` whose checkpoint row was inserted since the pass before the previous one (the same overlap as for tombstones) it deletes all rows with `_version` below the checkpoint's `indexed_at`, which removes the previous snapshot, orphan rows of failed runs and the cleaner's tombstones in one statement per table, pruned by the primary key to the affected projects. A scope without a live `Branch` row at or after the checkpoint's `indexed_at` recorded an empty repository, possibly after a transient fetch failure, and is skipped. The first pass sweeps every scope whose file table holds more than one snapshot version, in chunks of 2,000; scopes with a single snapshot have nothing to remove. The target table is joined through a pre-filtered derived table, because joining it directly made ClickHouse read the whole table once more without a key condition. On the shared `gl_edge` table only paths with a single indexed branch are cleaned, bounded by that branch's checkpoint, and only rows of the code node kinds are touched.
