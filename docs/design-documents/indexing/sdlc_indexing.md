...
- KV buckets (`indexing_locks`) are keyed by graph schema version (e.g. `indexing_locks_v77`), so same-schema releases share locks while different-schema releases are isolated. Retired schema versions' buckets are deleted by the migration-completion GC.
