...
5. Transform each extracted block with the plan's transform. `datafusion` performs row-wise SQL projections for node rows and edge mappings; a registered Rust transform such as `system_notes` can perform custom parsing or lookups. Output rows are grouped by destination table. The extracted page is moved into the transform, so each block is released as it is consumed and none of it survives the transform. While the current page's writes are in flight, the next page's read is overlapped via `tokio::join!`, so the next page's query-open latency hides behind the writes.
   A DataFusion projection's output aliases the extracted block's Arrow buffers, so the transformed page and the page it came from are the same bytes; each table's batches are released as soon as they have been encoded into the insert body, so residency during the overlap is the next extracted page plus whatever of the current page is still being encoded, not two full pages.
6. Each destination table's transformed rows for a page are written as one bulk `INSERT` per page — the trade for throughput on high-latency backends like ClickHouse Cloud, where one large insert per page beats many smaller round-trips.
   The insert streams the page block by block and drops each Arrow batch once its bytes are in the request body, so only the compressed body, not the Arrow page, waits for the server's acknowledgement. Read-side wire blocks use the ClickHouse server default `max_block_size`; only the 1002 overflow retry (point 4) overrides it per attempt.
