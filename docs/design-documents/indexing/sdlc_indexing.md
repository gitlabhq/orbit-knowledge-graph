...
Rows deleted in the source database have `_siphon_deleted` set to `true`. The extraction query pulls these rows alongside live data, and the `_deleted` flag carries through to the graph table. Downstream queries filter on the flag.

Tombstoned rows are reclaimed physically through the stale row ledger. Whenever the Arrow writer inserts a batch with `_deleted = true` rows into a graph table, it appends one row per tombstoned key to the unversioned `stale_rows` table: the logical table name, the sort key rendered as a SQL tuple literal, and the tombstone `_version`. The dispatcher's `maintenance.stale_reclaim` task (hourly by default) reads the pending ledger rows per table in `retired_at` order, confirms with one primary-key lookup that each key's newest tombstone is strictly newer than its newest live row, and removes the confirmed keys with a single lightweight `DELETE` whose predicate lists the exact (sort key, `_version`) tuples: the live rows in one pass and the tombstones one pass later, once the previous mutation has finished. A key that was re-created after its tombstone, or whose live row ties with the tombstone, is skipped, so a live row is never removed. The statement is shared with the code snapshot ledger (see the code indexing design document), so each table receives at most one mutation per pass, and a table with an unfinished mutation is skipped until its queue drains.
...
A dual `IN` on `(traversal_path, owner-id)` prunes the edge scan to the changed set via the primary key, so cost tracks churn rather than table size; the cursor advances only on full success, and re-tombstoning an already-stale edge is a no-op. After the tombstone insert, a second statement appends the tombstones it just wrote (the deleted edges of those kinds at or above the cursor for the changed owners) to the `stale_rows` ledger, so the reclaim task removes them physically like any other tombstoned row.
