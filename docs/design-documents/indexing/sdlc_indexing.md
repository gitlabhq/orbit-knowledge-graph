...
A simple FK relationship is declared once, graph-natively, under `edges:`. Each entry is two endpoints `{node, key}` where one `key` is `id` (the anchor) and the other is the foreign-key **graph** column (the owner). From that single entry the loader derives both the query entity (`fk_column` = the graph key) and the ETL node binding — and it resolves the binding's **datalake** column through the owning node's property map (e.g. `MergeRequest.closed_by_id` → siphon `metric_latest_closed_by_id`), so the siphon name lives only on the node, never in the edge file. A file-level `scope:` applies to every entry (overridable per entry).

Producers that aren't a single node FK stay under `sources:`: `table:` sources (one edge per join-table row), `node:` sources with `unnest:` (array column) or a polymorphic `type_column`, and `transform:` sources (emitted by a derived entity's Rust transform). The query-only `variants:` form remains for relationships with no datalake producer (code-graph edges). At load time, node bindings (from `edges:` or `node:` sources) are folded into the producing node's plan so they share the same extracted batch instead of querying the datalake twice; table sources get their own plans. Plans split by `EtlScope` into global (instance-wide entities like User) and namespaced (entities under a namespace like Project or Issue).
