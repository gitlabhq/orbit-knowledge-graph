...
Each `EntityHandler` invocation runs its plan through a shared `Pipeline` struct. The pipeline is a typed page-granular state machine (`crates/indexer/src/modules/sdlc/pipeline/`): page payloads (`ExtractedPage` → `TransformedPage` → `StagedWrites`) are the states, the producer/processor/consumer stages under `pipeline/stages/` are the transitions, and the runner in `pipeline/mod.rs` composes them per page. The loop works like this:
