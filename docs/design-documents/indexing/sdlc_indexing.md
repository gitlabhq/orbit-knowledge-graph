...
ETL plans come from ontology pipelines declared on nodes, edges, and derived entities in `config/ontology/nodes/`, `config/ontology/edges/`, and `config/ontology/derived/`. A pipeline has `name`, `extract`, and `transform` sections.
Each node pipeline becomes a `Plan` with an extraction query and one or more transforms; the FK edges in its `transform.edges` list are folded into the node's plan so they share the same extracted batch instead of querying the datalake twice. Edge pipelines get their own plans (named after the composed checkpoint key, e.g. `ASSIGNED_siphon_merge_request_assignees_MergeRequest`); derived-entity pipelines emit no node rows and must name a Rust transform. Plans split by `EtlScope` into global (instance-wide entities like User) and namespaced (entities under a namespace like Project or Issue).

Each `extract` declares a `type` (currently only `clickhouse`, a required field so a future non-ClickHouse source has a seam), its source `tables` (the first is the extraction source and partition-probe base), keyset `order_by`, and a `query`. Two forms of `query`:

- `query: generated` — the extraction SQL is a projection of the source table built from the declaration. Endpoint enrichment on a standalone edge is declared under `transform.edges` (`from`/`to` `enrich:`) and becomes `_eN` point-lookup CTEs against each endpoint node's datalake source — the edge YAML never restates the join.
- an authored query object — explicit `select`/`from`/`where` pieces (plus optional `watermark`/`deleted` overrides, `traversal_path_filter`, `table_alias`, and a `page_join`) for entities whose extraction owns multi-table joins or computed columns that are not mechanically derivable (Group, Project, MergeRequest, Commit, MergeRequestDiffFile, Finding, PackageFile, Note, and the SystemNote derived entity). The loader resolves `{{watermark_column}}`/`{{deleted_column}}` placeholders at load time and rejects hardcoded column names.

A standalone edge pipeline may set an optional `extract.filter` predicate that is pushed into the extraction SQL (ANDed with the watermark/deleted/not-null filters), so a single shared source table can back multiple edge kinds without a dedicated table or a Rust transform. For example, `REOPENED` is two pipelines over `siphon_resource_state_events` filtered to `state = 5` (the Rails `reopened` issuable state) — one targeting `MergeRequest` via `merge_request_id`, one targeting `WorkItem` via `issue_id`.
