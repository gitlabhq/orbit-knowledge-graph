...
- a `.sql.j2` MiniJinja template next to the YAML — the seven genuinely complex nodes (Group, Project, MergeRequest, Commit, MergeRequestDiffFile, PackageFile, Finding) whose extracts own multi-table joins, materialized arrays, or hex/SHA decoding that are not mechanically derivable, and the SystemNote derived entity. Authored lookup CTEs must collapse to one row per key with `argMax` over the version column like the generated ones do; a plain join against a ReplacingMergeTree table multiplies the page by the number of unmerged versions and defeats the `LIMIT`. Derived-entity pipelines are always authored SQL: their rows are neither node properties nor edge endpoints, so there is nothing to generate a projection from.
...
5. Transform each extracted block with the plan's transform. `datafusion` performs row-wise SQL projections for node rows and edge mappings; a registered Rust transform such as `system_notes` can perform custom parsing or lookups. Output rows are grouped by destination table. The extracted page is moved into the transform, so each block is released as it is consumed and none of it survives the transform. While the current page's writes are in flight, the next page's read is overlapped via `tokio::join!`, so the next page's query-open latency hides behind the writes.
   A DataFusion projection's output aliases the extracted block's Arrow buffers, so the transformed page and the page it came from are the same bytes; each table's batches are released as soon as they have been encoded into the insert body, so residency during the overlap is the next extracted page plus whatever of the current page is still being encoded, not two full pages. Decoded blocks are also rebuilt so that no column keeps a slice of the compressed IPC message body alive after decoding.
6. Each destination table's transformed rows for a page are written as one bulk `INSERT` per page — the trade for throughput on high-latency backends like ClickHouse Cloud, where one large insert per page beats many smaller round-trips. The insert streams the page block by block and drops each Arrow batch once its bytes are in the request body, so only the compressed body, not the Arrow page, waits for the server's acknowledgement. Edge outputs are not sorted in-process; ClickHouse orders each inserted part by the table's sort key itself. Read-side wire blocks use the ClickHouse server default `max_block_size`; only the 1002 overflow retry (point 4) overrides it per attempt.
