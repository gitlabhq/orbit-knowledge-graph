...
- **Tenant isolation:** Per-tenant ClickHouse schema.  Three-layer auth stack:
  org isolation, traversal ID filtering (prefix-tree, Redis-cached), and
  Rails redaction via ``Ability.allowed?``.
...
- **DAP integration:** LLM agents access Orbit via DAP with two tools
  (``get_graph_entities``, ``execute_query``).  Progressive schema disclosure
