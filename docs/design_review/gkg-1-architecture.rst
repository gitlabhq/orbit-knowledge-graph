...
For the standalone desktop binary (embedded Ladybug DB, no cloud
...
.. list-table::
   ...
   * - **SDLC Indexer**
     - Rust ETL
     - Replicates SDLC entities (issues, MRs, pipelines, vulnerabilities,
   ...
   * - **Code Indexer**
     ...
     - Fetches source from Gitaly, parses with language-specific analyzers,
       ...
       shared ontology.
   * - **Graph Query Engine**
     - Rust (4-phase compiler)
     - Compiles untrusted JSON DSL into parameterized ClickHouse SQL.
       Five query types: multi-hop traversal, search, aggregation,
       pathfinding, nearest neighbors.  Auth via traversal_path filter
       injection.  Semi-Join Pushdown, edge-centric traversal, keyset
       pagination.
   * - **HTTP/gRPC Server**
     - Axum 0.8
     - REST API (query, schema, status, tools), MCP endpoint (JSON-RPC 2.0),
       gRPC (5 RPCs with stub caching), JWT auth middleware.
   * - **Orbit Dashboard**
...
.. list-table::
   ...
   * - **DAP / AI Gateway**
     - HTTP + JWT
     - DAP -> Orbit
     - Agent tool calls (``get_graph_entities``, ``execute_query``)
   * - **Rails gRPC Client**
     - gRPC
     - Rails -> Orbit
     - 5 RPC methods with stub caching
   * - **Snowplow -> CDot**
     - Event emission
     - Orbit -> Snowplow
...
- Redis (auth cache --- already part of GitLab infrastructure)
