...
Use the Orbit query language to get data from the knowledge graph. The language
is JSON so it can be written by humans, generated by AI agents, and validated
before Orbit compiles it to SQL.

You can use the query language with:

- The Orbit dashboard query editor.
- The [Orbit API](https://docs.gitlab.com/api/orbit/).
- [Orbit MCP tools](mcp_tools.md).
- The [local Orbit indexer developer preview](../local_indexer.md).
