...
| `schedule.tasks.table-cleanup.max_candidates_per_statement` | `2000000` | Tombstoned keys per delete statement. Tables with a traversal path column are grouped by path up to this many keys and 500 paths per statement; a single path above the limit is split by key hash; tables without a path column are split by key hash |
