...
| `gitlab.gitaly_proxy.webserver_max_sessions` | `GKG_GITLAB__GITALY_PROXY__WEBSERVER_MAX_SESSIONS` | `64` | Upper bound for retained sessions plus capacity-evicted sessions still held by active streams |
...
| `gitlab.gitaly_proxy.webserver_negative_cache_capacity` | `GKG_GITLAB__GITALY_PROXY__WEBSERVER_NEGATIVE_CACHE_CAPACITY` | `1024` | Maximum project cooldown entries retained by one webserver |
