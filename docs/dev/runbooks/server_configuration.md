...
| Stale reclaim | `schedule.tasks.stale-reclaim.cron` | `0 0 * * * *` (hourly) | Removes retired code snapshots and tombstoned SDLC rows recorded in the stale ledgers with one lightweight `DELETE` per graph table; skips a table while it has an unfinished mutation. Each pass re-reads the predicate columns of every part holding a matched row, so a shorter cadence costs IO on large tables |
...
| Config path | Default | Description |
|-------------|---------|-------------|
| `schedule.tasks.stale-reclaim.max_keys_per_statement` | `5000` | Ledger rows read per table, ledger and phase per pass |
| `schedule.tasks.stale-reclaim.max_statement_bytes` | `524288` | Byte cap for the tuple lists of one `DELETE` across both ledgers and phases; rows that do not fit wait for the next pass |
| `schedule.tasks.stale-reclaim.max_unfinished_mutations` | `0` | Unfinished mutations a table may have before the pass skips it; skips count in `gkg.scheduler.task.requests.skipped` |
| `schedule.tasks.stale-reclaim.settle_secs` | `30` | Ledger rows younger than this are left for the next pass |
