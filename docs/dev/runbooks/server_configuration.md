...
1. Retries a request that fails with error 286 (`UNSATISFIED_QUORUM`) or 289 (`REPLICA_IS_NOT_IN_QUORUM`) with a linear backoff of 100 ms per attempt, capped at 1 s, up to 20 attempts. Both errors are transient by design: serialized quorum inserts collide, and a sequential-consistency read can land on a replica that has not received the last quorum write. The load balancer moves the retry to another replica.
