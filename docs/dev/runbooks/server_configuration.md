...
| `schedule.tasks.table-cleanup.max_candidates_per_statement` | `2000000` | Tombstoned keys per delete statement. Tables with a traversal path column are grouped by path up to this many keys and an eighth of the session's `max_query_size` in path literals per statement (32 KiB under the ClickHouse default, at most 1 MiB); a single path above the limit is split by key hash; tables without a path column are split by key hash |
...
| `schedule.tasks.table-cleanup.sweep_history` | `true` | Remove every existing tombstoned key and every project with more than one code snapshot the first time a table is seen; `false` starts from the current parts and leaves history to re-indexes and the daily purge |
| `schedule.tasks.table-cleanup.concurrent_tables` | `4` | Tables whose tombstone collapse runs at the same time; the first sweep after a rebuild is bounded by the slowest table rather than the sum |
| `schedule.tasks.table-cleanup.merges_cluster` | `default` | Cluster whose `system.merges` are read through `clusterAllReplicas` to exclude parts that are merging on any replica from every delete; when the cluster is not defined only the local replica's merges are excluded |
...
The task waits while the pinned schema version is still migrating and starts with a full sweep once it is active. The writer role needs `SELECT` on `system.parts`, `system.parts_columns`, `system.tables`, `system.settings`, `system.mutations`, `system.merges` and `system.clusters` for these checks, plus `REMOTE ON *.*` to read `system.merges` of the other replicas. A table with a pending mutation is skipped for the pass and parts of merges in flight are excluded from each statement; the rows left in them are caught up by the first pass that finds the table quiet, so a long merge of a large part shows up as a `hold` in the table's cursor rather than as an error. Progress is stored in the `checkpoint` table under `maintenance.table_cleanup.<versioned table>`; deleting a table's row re-runs its history sweep on the next pass.

The identity verdict is stored under `maintenance.table_cleanup.identity.<versioned table>` for the whole schema version; delete it to re-check a table that was rebuilt. Alert on `gkg.scheduler.task.errors{task="maintenance.table_cleanup"}` (stages `prepare`, `identity`, `code_snapshots`, `code_history`, `collapse`, `apply_patches`); skipped tables and unsupported servers count on `gkg.scheduler.task.requests.skipped{task="maintenance.table_cleanup"}`; each delete statement's duration lands on `gkg.scheduler.task.query.duration` with the versioned table name as the `query` label.
