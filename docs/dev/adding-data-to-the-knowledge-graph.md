...
This rewrites `config/graph.sql` (remote/ClickHouse) and
`config/graph_local.sql` (local/DuckDB). Materialized-view SELECT templates
under `config/ontology/sql/` are loaded directly by the ontology. Expect:
...
- `config/ontology/sql/namespace_storage_snapshot.sql`: a non-global node or edge table adds one namespace-storage attribution branch; global nodes are aggregated under `__global`.
