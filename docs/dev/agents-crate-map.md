...
| `memprof` | Dev-only RSS-correlated allocation profiler: a `GlobalAlloc` shim records sampled (>= threshold) allocation backtraces while a background thread samples process RSS; at drop it replays events up to the RSS peak and reports live bytes by call site, writing a CSV timeline and an HTML chart. Wired into `orbit-local` behind its `memprof` feature. Does not inherit `[workspace.lints]` (the shim needs `unsafe`). |
