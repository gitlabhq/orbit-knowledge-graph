...
| `orbit-search` | Search for `orbit ask`: backend-neutral model (identifier tokenizer with camelCase split + Snowball stem, query-term processing, `SearchVocab` edge-kind vocabulary, tiered ranking with per-term seed guarantees and dedupe caps) plus shared corpus policy (`corpus`) and the feature-gated DuckDB fetch impl (`duckdb`: postings build, BM25 candidates, edge expansion). A future remote fetch lives server-side behind query-engine/redaction and consumes the model only |
