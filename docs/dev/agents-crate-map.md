...
| `query-engine/gql` | Read-only ISO GQL frontend: rust-peg grammar whose actions assemble the compiler's `Input` directly, `limits` enforces the crate's own complexity caps, then `compile_structured` runs the unchanged pass pipeline |
