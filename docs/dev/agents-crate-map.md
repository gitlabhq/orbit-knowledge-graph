...
| `object-storage` | Builds an `object_store` client (AWS S3, S3-compatible, GCS) from `ObjectStorageConfig`, plus the `probe` binary that round-trips a config file against a real bucket |
