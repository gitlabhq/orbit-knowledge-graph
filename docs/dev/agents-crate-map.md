...
| `orbit-rerank` | Cross-encoder reranking for `orbit ask`: loads a sentence-transformers MS MARCO BERT export (`config.json`, f32 or f16 `model.safetensors`, `vocab.txt`) through candle on the CPU with a built-in uncased WordPiece tokenizer, and scores (question, definition passage) pairs; implements the `orbit-search` `Reranker` hook via the CLI adapter in `orbit-local`. Released binaries embed `cross-encoder/ms-marco-MiniLM-L2-v2` as f16 (~31MB) via `cargo xtask rerank-bundle` + the `orbit-cli/bundled-reranker` feature; dev builds download it to `~/.orbit/models/` on first use |
