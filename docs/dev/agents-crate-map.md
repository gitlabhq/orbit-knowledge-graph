...
| `gkg-prompts` | Proc-macro prompt registry: `embed!("remote"\|"local")` compiles the versioned YAML prompt files under `config/prompts/` into modules of string constants at expansion time, validating semver versions, short/description prefix agreement, and MiniJinja template placeholders against each prompt's declared `variables:` |
