...
| `named-queries` | Named queries: parses, validates, and embeds JSON and GQL spellings from `config/named_queries/`; validates client parameters against JSON Schema; keeps server-derived bindings separate; and renders JSON placeholders or GQL lookup functions. `orbit-server` compiles both examples at build time. Existing callers keep the JSON rendering API, while language-aware methods expose either spelling. |
...
| `orbit-search` | Shared local search result types, case-deduplicated OR alternatives, and corpus policy (source extensions, test/vendor exclusions). DuckDB FTS retrieval, BM25 ordering, scopes, and SQL result limits live in `duckdb-client::search` |
