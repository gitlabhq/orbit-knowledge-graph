...
| `orbit-rerank` | Cross-encoder reranking for `orbit ask`: loads a sentence-transformers BERT export (`config.json`, f32 or f16 `model.safetensors`, `tokenizer.json`) through candle on the CPU, tokenizes with the `tokenizers` crate, and scores (question, definition passage) pairs; implements the `orbit-search` `Reranker` hook via the CLI adapter in `orbit-local`. The model (`cross-encoder/ms-marco-MiniLM-L2-v2`, pinned revision, sha256-verified, converted to f16 by `cargo xtask rerank-bundle`) is embedded at build time when `target/rerank-bundle` exists (`cfg(orbit_reranker_bundle)` from `build.rs`); `ORBIT_RERANK_MODEL_DIR` overrides it with a local export, and a build without the bundle prints a note and keeps lexical order |
| `orbit-search` | Store-agnostic graph search ranking: recall stays behind the `AskSource` trait (`recall(term) → (id, sim)`; DuckDB FTS (BM25) locally, orbit-next remotely later), sim-based match scoring with coverage and weak-match signaling (`rank`), ObjectRank ranking over whatever graph the `GraphSource` hands over (local: the whole project graph; a remote impl may return an authorized neighborhood) — per-term personalized PageRank combined by quorum consensus (`ppr`, `expand`), ontology-derived relational-verb vocab (`vocab`), and the ask orchestration (`ask`). Shared corpus policy in `corpus` |
