...
| Crate | Role |
|---|---|
| `orbit-server` | HTTP/gRPC server, all 4 modes, JWT auth, config loading, archive-backed schema serving and readiness (`schema_watcher.rs`), MCP tool registry, and Orbit agent command registry (`CommandRegistry`) |
