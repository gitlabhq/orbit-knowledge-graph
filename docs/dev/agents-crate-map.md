...
| `gkg-prompts` | Agent-facing prompt registry: loads and validates the versioned YAML prompt files under `config/prompts/` and renders each into Rust string constants; consumed by the `gkg-server` and `orbit-local` build scripts so tool descriptions compile into the binaries |
