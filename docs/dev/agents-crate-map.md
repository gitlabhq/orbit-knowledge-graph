...
| `code-index-profiler` | Dev-only binary that drives the production code-indexing handler (archive download -> extract -> parse -> Arrow -> ClickHouse) against a local corpus archive server and a throwaway ClickHouse database, under a sharded tracking allocator, a `phys_footprint`/RSS sampler, and optional dhat heap profiling. See `docs/dev/runbooks/code_indexing_memory_profiling.md`. |
