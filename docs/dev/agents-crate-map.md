...
| `sdlc-profiler` | Standalone CLI that seeds a production-scale synthetic siphon datalake in ClickHouse and profiles the SDLC indexer's backfill (extract, transform, graph write) for peak memory |
| `memprofile` | Shared memory-profiling sampler for the profiler CLIs: tracking global allocator (sharded requested-byte counters), RSS/footprint/live-bytes timeline, exact kernel and mimalloc high-water marks |
