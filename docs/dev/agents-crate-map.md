...
| `orbit-search` | Search model for `orbit ask`, dependency-free so writers can embed it: identifier tokenizer with camelCase split + Snowball stem (`search_document` feeds `gl_definition.search_text`), query-term processing, `SearchVocab` edge-kind vocabulary, tiered ranking with per-term seed guarantees and dedupe caps, shared corpus policy (`corpus`). A future remote fetch lives server-side behind query-engine/redaction and consumes the model only |
| `duckdb-client` | DuckDB client with read-write retry backoff, read-only concurrent access, ontology-driven graph converter, shared SQL-literal/Arrow-column helpers, and the local search executor (`search`: BM25 over `gl_definition.search_text`, edge expansion) |
