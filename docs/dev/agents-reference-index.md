...
| Orbit skill sources, `orbit skills [name] [path]` command, and composition guardrails | `skills/orbit/` (remote), `skills/orbit-cli/` (local, with references under `references/local/`), `crates/orbit-cli/src/skill.rs` (listing and argument disambiguation), and `crates/orbit-skill/src/validation.rs` (build-time marker, path, link, and command validation) |
