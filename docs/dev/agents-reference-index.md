...
| Denormalized join tables (`denormalized: true` edge variants) | `crates/ontology/src/denormalized.rs` (naming, column contract, sort key), `crates/query-engine/compiler/src/passes/codegen/ddl/denormalized.rs` (table and feeding views composed from the source tables' generated DDL), `passes/plan/edge_chain.rs` (`bind_denormalized`) and `passes/lower/rebind.rs` (alias rebinding); design in `docs/design-documents/querying/graph_engine.md` |
