...
| Denormalized paths (`settings.denormalized_paths` in `schema.yaml`) | `crates/ontology/src/denormalized.rs` (naming, position column contract, sort key), `crates/ontology/src/loading/mod.rs` (`resolve_denormalized_path`), `crates/query-engine/compiler/src/passes/codegen/ddl/denormalized.rs` (table and feeding views composed from the source tables' generated DDL); design in `docs/design-documents/querying/graph_engine.md` |
