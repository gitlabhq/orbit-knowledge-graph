...
| Pinned versions | `config/versions.yaml` (flat `key: value`; `schema` u32 bumped via `mise schema:bump`; `query_dsl`, `raw_output_format`, `goon_output_format` semvers enforced by `scripts/check-pinned-version.sh <pin>`; `duckdb` release; `gitlab_system_note_actions` upstream SHA; embedded at compile time as `orbit_utils::pinned::VERSIONS`) |
