...
| Unversioned graph DDL (ClickHouse) | `config/graph_unversioned.sql` (generated by `cargo xtask ddl --target unversioned`; namespace-storage snapshot table plus a version-prefixed refreshable MV; applied by the indexer at startup and after promotion) |
