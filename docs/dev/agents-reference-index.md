...
| Orbit skill sources and composition guardrails | `skills/orbit/` (remote), `skills/orbit-cli/` (local, with references under `references/local/`), and `crates/orbit-skill/src/validation.rs` (build-time marker, path, link, and command validation) |
