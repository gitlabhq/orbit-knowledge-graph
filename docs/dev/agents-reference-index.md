...
| Orbit skill sources, `orbit skills [name] [path]` command, and build checks | `skills/orbit/` (remote), `skills/orbit-cli/` (local, with references under `references/local/`), `crates/orbit-cli/src/skill.rs` (listing and argument disambiguation), and `crates/orbit-skill/src/validation.rs` (build-time placeholder, local-section, path, link, and command validation) |
