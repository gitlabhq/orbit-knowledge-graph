...
| Agent prompt files (tool descriptions) | `config/prompts/` (versioned YAML, one file per prompt; `remote/` feeds `gkg-server`, `local/` feeds `orbit-local`, embedded at compile time via the `gkg-prompts` proc macro) |
