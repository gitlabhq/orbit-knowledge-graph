...
- **A graph-shape fact belongs in the ontology, declared once — not mirrored in Rust.** Before adding a Rust flag, config field, ETL tag, or constant that encodes a node/edge property (a node is global, an edge's scope, which physical table an edge routes to, etc.), check whether the ontology YAML already declares it — or is the right place to declare it. Reviewers repeatedly ask "isn't this already in the ontology?" and "this duplicates the ETL tag" (!1829, !1837, !1835). If the same fact ends up declared in two places, delete one; the ontology is the single source of truth and ETL/query validation/redaction all read from it.
...
Language-specific code belongs in `crates/code-graph/src/v2/langs/{generic,custom}/`, not in the shared linker, pipeline, or analyzer. The linker and the generic pipeline are performance-sensitive production code — introducing per-language branches there increases complexity for all languages. When a language needs special handling, express it as a **generic hook** the pipeline already exposes (e.g. a generic import/scope hook applied per language via config) or reuse an existing edge type rather than a language branch in shared code — invert `Shape -PERMITS-> Circle` into `Circle -EXTENDS/IMPLEMENTS-> Shape` instead of adding a Java-specific `PERMITS` path to the linker (!1725), and add a generic import hook rather than a Python-specific exception in the pipeline (!1890). When adding a new language, include a `code-indexing-benchmark.yaml` entry and test fixture repos under `fixtures/code/` so the team can evaluate coverage.
