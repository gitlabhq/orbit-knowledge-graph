...
- **Ontology-driven graph.** YAML in `config/ontology/nodes/` and `config/ontology/edges/` drives ETL, query validation, redaction, and edge table routing. New entity types start there, not in Rust. Node `etl:` blocks are two-tier: declarative (`source`/`scope`/`filter`) or a complete SELECT in a sibling `query: <file>.sql` (using `{{watermark_column}}`/`{{deleted_column}}` placeholders). A simple FK relationship is declared once under `edges:` as two `{node, key}` endpoints — one `key` is `id`, the other is the foreign-key graph column; the loader derives both the query entity and the ETL node binding from it, resolving the datalake column through the owning node's property map (so siphon names live only on the node). Producers that aren't a single node FK stay under `sources:` (`table:` join-table rows, `node:` `unnest:`/polymorphic bindings, `transform:` Rust transforms); `variants:` remains for query-only edges with no datalake producer (code-graph). Edge YAML top-level `table:` field + `settings.edge_tables` in `schema.yaml` control which physical table each relationship type writes to and queries from (default: `gl_edge`). Schema: `config/schemas/ontology.schema.json`.
