...
- **Orbit query frontend.** `crates/query-engine/compiler/src/passes/frontend/` holds one module per query language; each yields a language-neutral `Statement`. `compiler::prepare` parses once and dispatches MATCH to the complete shared graph compilation phases and standalone `CALL db.schema(...)` to the `schema_call` pipeline, which resolves typed ontology metadata without SQL. `compiler::compile` remains graph-query-only. Remote requests still use JSON. See `docs/design-documents/querying/orbit_query_frontend.md`.
