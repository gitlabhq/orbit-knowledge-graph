...
| `migration-framework` | Ontology-driven schema migrations. Today: **generation** (`generation` module) — diffs the ontology's desired DDL against a baseline and emits an additive migration (`CREATE TABLE` for new entities, `ADD COLUMN`/`ADD INDEX`/`ADD PROJECTION` for drifted ones), rejecting non-additive drift. Runner/ledger land here later. See ADR 016. |
