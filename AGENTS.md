...
- **Schema archives.** Schema bumps retain one ontology archive per version. The dispatcher publishes its archive and requires the active archive before migration. Promotion validates the target archive; promotion and rollback replace active/retired statuses in one write. Existing installations need an archive-publishing release before upgrading; serving stays unchanged. See `docs/design-documents/schema_management.md`.
