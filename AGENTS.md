...
- **Schema archives.** Schema bumps retain one ontology archive per version. The dispatcher publishes it before migration; serving stays unchanged. See `docs/design-documents/schema_management.md`.
...
- **Layered configuration.** `AppConfig` in `crates/orbit-server-config/` loads five sources (lowest to highest priority): the embedded `config/default.yaml` (compiled in via `include_str!`), an on-disk `config/default.yaml` when present (the Helm ConfigMap key), an overlay file (`--config <path>`, else `config/config.yaml` when present), K8s secret files from `/etc/secrets/`, and `GKG_*` environment variables (`__` separates nested keys, e.g. `GKG_GRAPH__DATABASE`). `config/default.yaml` is the single source of truth for default values: every section and scalar is declared there and the Rust structs have no `Default` impls or `serde(default)` fallbacks (only `Option` fields and empty collections may be omitted). Add a setting by adding the struct field and its value in `default.yaml`; tests start from `AppConfig::embedded_defaults()`. The CLI (`orbit`) has its own clap-based config and does not use `AppConfig`. See `docs/dev/runbooks/server_configuration.md` for full reference.
