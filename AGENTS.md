...
- **Schema upgrades retain ontology archives.** `mise schema:bump` writes `config/ontology-archives/v<N>.tar.gz`; the dispatcher publishes the current archive to durable NATS KV before migrating. Webservers load the database’s active version and atomically replace the serving snapshot at promotion, without a restart or blue-green deployment. Each RPC keeps one snapshot. Readiness depends on a usable active archive, not completion of the newest backfill. Existing installations need a preparatory archive-publishing release. See `docs/design-documents/schema_management.md`.
