...
| Iglu schemas (committed) | `config/schemas/iglu/<name>/<version>.json` (update via `mise iglu:bump -- <name> <version>`) |
...
| `gkg-analytics` | Consumer-owned Snowplow context types (`OrbitCommonContext`, `OrbitQueryContext`) and tracker infrastructure (`AnalyticsTracker` trait, `SnowplowAnalyticsTracker`, `InMemoryAnalyticsTracker`). Context types replace the deprecated `labkit_events::orbit::*` types and implement `labkit_events::SnowplowContext`. Schema URIs and full schema JSON are inlined at compile time by `crates/gkg-analytics/build.rs` from Iglu version pins (`config/schemas/iglu/<name>.version`) and the committed schema files (`config/schemas/iglu/<name>/<version>.json`). `load_schema_json()` returns the inlined JSON for test validation. |
