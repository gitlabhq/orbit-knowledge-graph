...
- **Signed images.** The `docker-manifest` and `release-manifest` jobs compose the multi-arch index from the digests the build jobs report. They sign every published digest with keyless cosign before the final tags move (`scripts/publish-manifest.sh`, `scripts/sign-image.sh`). Only the canonical project signs. Forks, including private forks, publish unsigned. See `docs/design-documents/security.md`.
