...
- **Keep ontology descriptions short.** Descriptions in `config/ontology/` are shown to users and agents, so they must be scannable and token-efficient. State what the node, edge, property, or domain represents in one sentence; move rationale, caveats, and examples to design docs or comments. CI enforces a 200-character maximum for ontology `description` fields.
