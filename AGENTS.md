...
- **Schema archives.** Migration and promotion require usable ontology archives. The Webserver serves the active archive, using embedded bytes only when its migration version matches. Each schema-dependent request pins an immutable `ServingSchema`: migration version, prefixed ontology, compatible named queries, and a per-schema path resolver. `ToolService` validates commands into typed plans; schema rendering uses a pinned snapshot, while static commands need none. The watcher builds, rechecks active, and atomically swaps snapshots; failures retry without restart. Readiness tracks snapshot availability, independently of liveness. See `docs/design-documents/schema_management.md` for failure handling, rollout prerequisites, and retained-table limits.
