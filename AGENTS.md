...
- **Orbit skills remain standalone.** `skills/orbit/` owns remote guidance and `skills/orbit-cli/` owns the binary-embedded local fallback, whose references live under `references/local/`. Matching remote placeholders and local sections, unique combined paths, links, and commands are build-time validated by `orbit-skill`; the current CLI still serves only its embedded local tree.
