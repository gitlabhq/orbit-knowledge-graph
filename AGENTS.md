...
- **Keep ontology descriptions short.** Node, edge, and property descriptions are surfaced through schema/introspection paths and must be scannable and token-efficient. State what the ontology item represents in one sentence; move rationale, caveats, and examples to YAML comments or design docs. CI enforces a flat 200-character maximum for every ontology `description` field, including human-facing top-level descriptions.
