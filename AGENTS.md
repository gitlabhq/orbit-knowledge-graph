...
- **Ontology-driven graph.** YAML in `config/ontology/nodes/`, `config/ontology/edges/`, and `config/ontology/derived/` drives ETL, query validation, redaction, and edge table routing.
  ...
  At runtime, extraction sessions emit Arrow `RecordBatch` pages with opaque source resume state. The shared pipeline transforms and bulk-writes those pages while overlapping page N+1 extraction with page N writes. ClickHouse-specific SQL, keyset cursors, partitioning, retries, and checkpoint encoding stay inside `ClickHouseExtractor`, so another extractor can reuse the same DataFusion or Rust transform without importing ClickHouse runtime types.
