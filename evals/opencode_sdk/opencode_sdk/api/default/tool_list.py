...
) -> BadRequestError | list[ToolListItem] | None:
    if response.status_code == 200:
        response_200 = []
        _response_200 = response.json()
        for componentsschemas_tool_list_item_data in _response_200:
            componentsschemas_tool_list_item = ToolListItem.from_dict(
                componentsschemas_tool_list_item_data
            )
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[BadRequestError | list[ToolListItem]]:
    return Response(
...
def sync(
    *,
    ...
    Returns:
        BadRequestError | list[ToolListItem]
    """
