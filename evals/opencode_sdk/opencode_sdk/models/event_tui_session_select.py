...
class EventTuiSessionSelect:
    """
    Attributes:
        type_ (Literal['tui.session.select']):
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        d = dict(src_dict)
        type_ = cast("Literal['tui.session.select']", d.pop("type"))
        if type_ != "tui.session.select":
            raise ValueError(f"type must match const 'tui.session.select', got '{type_}'")
        ...
        event_tui_session_select.additional_properties = d
