-- Materialized join table: REVIEWER (User -> MergeRequest)
...
-- 1. Target table
CREATE TABLE IF NOT EXISTS v93_mat_reviewer (
  -- relationship
...
ORDER BY (traversal_path, mr_id, u_id)
SETTINGS index_granularity = 1024;

-- 2. MV: fires on edge insert
CREATE MATERIALIZED VIEW IF NOT EXISTS v93_mat_reviewer_on_edge
TO v93_mat_reviewer AS
SELECT
...
CREATE MATERIALIZED VIEW IF NOT EXISTS v93_mat_reviewer_on_mr
TO v93_mat_reviewer AS
SELECT
  e.traversal_path AS traversal_path,
  e.relationship_kind AS relationship_kind,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS v93_mat_reviewer_on_user
TO v93_mat_reviewer AS
SELECT
  e.traversal_path AS traversal_path,
  e.relationship_kind AS relationship_kind,
