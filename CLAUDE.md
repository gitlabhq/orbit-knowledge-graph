...
- **Orbit query frontend.** `crates/query-engine/compiler/src/passes/frontend/` holds one module per query language; both lower graph queries to language-neutral `Input`. `compiler::gql::prepare` parses once and dispatches MATCH to the complete shared graph compilation phases. Standalone `CALL db.schema(...)` resolves typed ontology metadata inside the GQL frontend without SQL or shared schema state. `compiler::compile` remains graph-query-only. Remote queries accept `language: gql`; omitted language and named queries retain JSON. The server routing stage returns schema metadata directly for `CALL db.schema(...)`; MATCH continues through the graph pipeline. See `docs/design-documents/querying/orbit_query_frontend.md`.
- **Query result encoding.** Graph queries return `format_version`, `query_type`, `nodes`, and `edges`; aggregations also include `columns`, `group_columns`, and `rows`, with pagination metadata when applicable. `raw` is JSON; `llm` is standard TOON of the same payload. The query CLI defaults to `llm`; schema and health defaults are unchanged. See `docs/design-documents/querying/graph_engine.md`.
...
- Pins in `config/versions.yaml` bumped when their covered files change: query DSL, RAW response format, TOON encoding (`pinned-version-check`, one job reporting every stale pin at once)
