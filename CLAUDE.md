...
- **A graph-shape fact belongs in the ontology, declared once.** Do not mirror it in Rust. You may want to add a Rust flag, config field, ETL tag, or constant. Such a value can encode a node or edge property, such as global-ness, scope, or table routing. First check whether the ontology YAML already declares it, or should. The ontology is the single source of truth. ETL, query validation, and redaction all read from it. If the same fact lands in two places, delete one.
