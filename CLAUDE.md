...
- **Initial backfill status.** `graph_status.backfill` reads one root-namespace snapshot from unversioned NATS KV `orbit_indexing_progress`, keyed `backfill.<root_namespace_id>`. The dispatcher records completion after initial SDLC and all currently replicated projects are indexed, using target-schema checkpoints. Code completed counts are informational; no code total is exposed. Completed snapshots survive later indexing and rebuilds; root data deletion resets them, subgroup deletion does not. Missing status is unknown, not proof indexing never ran. See `docs/design-documents/decisions/010_graph_status_endpoint.md`.
