...
- **Orbit query frontend.** `crates/query-engine/orbit-query/` implements Orbit's read-only graph language with Pest and lowers pairs directly into compiler `Input`. It is a compiler-level API; remote requests still use JSON. See `docs/design-documents/querying/orbit_query_frontend.md`.
