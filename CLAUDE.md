...
- **Schema archives.** Migration and promotion require usable ontology archives. See `docs/design-documents/schema_management.md` for publication, rollout prerequisites, and rollback retention. Serving is unchanged.
...
- **Layered configuration.** `AppConfig` in `crates/orbit-server-config/` loads four sources (lowest to highest priority): the embedded `config/default.yaml` (compiled in via `include_str!`), an on-disk `config/default.yaml` when present (the Helm ConfigMap key), overlay files (each `--config <path>` in order, else `config/config.yaml` when present), and K8s secret files from `/etc/secrets/`. There is no environment-variable layer; the mise dev tasks layer `config/dev.yaml`, `config/dev/<mode>.yaml`, the generated `.dev/gdk.yaml`, and the Git-ignored `config/dev.local.yaml`.
