...
- **Signed images.** `docker-manifest` and `release-manifest` compose the multi-arch index from the digests the build jobs report and sign every published digest with keyless cosign before the final tags move (`scripts/publish-manifest.sh`, `scripts/sign-image.sh`). Only the canonical project signs; forks, including private forks, publish unsigned. See `docs/design-documents/security.md`.
