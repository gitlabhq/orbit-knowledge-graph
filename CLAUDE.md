...
- **One trust bundle for outbound TLS.** Every outbound client (GitLab API, ClickHouse, NATS, object storage) trusts the platform store plus the PEM bundle named by `tls.ca_bundle_path`, loaded once at startup by `crates/tls-trust` and passed to each client constructor; production code uses the `*_with_trust` constructors, the plain ones are platform-only for tests.
