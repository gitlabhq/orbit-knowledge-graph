...
- **Do not write narration comments — including in tests.** A comment must explain *why* (a non-obvious constraint, a gotcha, an ADR/issue link), never restate *what* the next line does. The most common leak is a label on each test or setup block; those are narration, delete them. The test name and the `assert_eq!` already say what is being checked. Write clean as you go. Clean-as-you-go alone has proven insufficient, so a final narration-comment pass over the comments your change added or modified is required before merge — run it before pushing or opening an MR. Discriminator:
  ...
  - ❌ A multi-line *why* paragraph above an inline block. Extract the block into a helper whose name carries the explanation (e.g. `receiver_types_for_resolver`) and keep at most one line of genuine *why*.
  - ❌ A `///` doc comment that restates the function name (`/// Drops entries flagged versioned: false` on `retain_versioned_entries`). If the signature already says it, omit the doc comment.
...
- **Keep tests lean: one test per behavior, not one per input.** Fold same-shape cases into a single table-driven test or a loop over `(input, expected)` pairs, and reuse the module's existing fixtures and helpers instead of re-building setup per test. Reviewers routinely ask to consolidate a fan of near-identical tests (nine down to two in one MR); write the consolidated form first.
- **Verify query-shape and SQL changes against real data, and say so in Testing.** `corpus-smoke-test` only proves every query in `fixtures/queries/corpus/` still compiles and runs — it asserts nothing about results. For changes to compiler passes, ontology SQL templates, indexer extract SQL, or edge/node routing: trigger the manual `e2e` CI job, run the affected queries (or the corpus, see `fixtures/queries/corpus/README.md`) against a data-rich namespace, and put a one-line summary plus the job link in the MR's Testing section. If a reviewer would have to ask "did you run this against prod-shaped data?", the evidence was missing.
...
For a change that spans layers (extract → transform → sink, compiler pass → codegen → response, CLI → auth → gRPC), add a short dataflow walkthrough or diagram inside the Agent context block — the `/dataflow-mapping` skill produces one. Reviewers ask for this on every multi-layer MR that ships without it; it is the difference between a same-day and a next-week review.

Any change under `docs/source/` is public-facing documentation. There is no assigned technical writer for Orbit, so request a Technical Writing review in the `#docs` Slack channel and add the `~documentation` label; TW checks style-guide conformance and where the page fits in the docs IA. Do not merge public docs on engineering approval alone.
