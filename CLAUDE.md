...
- **Schema archives.** Migration and promotion require usable ontology archives. The Webserver follows the active archive: embedded when its migration version matches, otherwise loaded from the catalog. Each schema-dependent request pins an immutable snapshot with the integer archive migration version, prefixed ontology, compatible named queries, and its own path resolver; `ToolService` receives that ontology per request. The watcher builds before atomically swapping, rechecks active before installation, and retries failures without an outdated-version shutdown. Readiness reflects snapshot availability; liveness and schema-independent RPCs are separate. See `docs/design-documents/schema_management.md` for failure handling, rollout prerequisites, and retained-table limits for in-flight requests.
...
- **Layered configuration.** `AppConfig` in `crates/orbit-server-config/` loads five sources (lowest to highest priority): the embedded `config/default.yaml` (compiled in via `include_str!`), an on-disk `config/default.yaml` when present (the Helm ConfigMap key), an overlay file (`--config <path>`, else `config/config.yaml` when present), K8s secret files from `/etc/secrets/`, and `GKG_*` environment variables (`__` separates nested keys, e.g. `GKG_GRAPH__DATABASE`).
